...
- **Git**: commit/push 전 TSC 0 errors 확인. 커밋 메시지 prefix `feat:`/`fix:`/`docs:`/`refactor:`/`chore:`. 한글 다량 시 `.commit-msg.tmp`+`git commit -F`(#17). 저위험·즉시확인 가능한 변경은 main 직접 push, 화면 UI 재설계·여러 파일 묶음 작업은 feature 브랜치(1인 개발이므로 PR 리뷰어 없이 본인 검토 후 직접 merge). **push 직후 `scripts/verify-vercel-deploy.sh --wait` 의무**(#36, main push 시) — exit 1 시 webhook 진단.
