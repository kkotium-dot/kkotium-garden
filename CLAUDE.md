...
- **합성 표준 권위문서**: 본품 무드 합성(추가이미지/상세 히어로)은 `docs/design/ADAPTIVE_COMPOSITE_ENGINE.md`(상품진실 앵커·3-Plane 리얼리즘·≥2무드·과대금지, 작업원칙 #61)를 권위로 따른다. 상품현실시트 작성 후 finish-image(C-3)·apply-composite(C-7)·개입카드(C-9) 경로.
