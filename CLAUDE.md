...
>
> 착수 전 필독: `docs/playbook/CORE_WORKING_PRINCIPLES.md`(3대 작업 방법론—출처교차검증·단건수습금지·전상품공통) · `docs/CORE_PRINCIPLES.md`(절대 금지·사람결정 관문) · `docs/DOMAIN_FACTS.md`(이 앱이 무엇인가) · `docs/PRODUCT_LIFECYCLE_FLOW.md`(상품이 지나는 길)
...
네이버 스마트스토어 셀러(스토어명: 꽃틔움 KKOTIUM, 운영자 본인이 스토어를 직접 운영)를 위한 운영 자동화 SaaS. 도매꾹/오너클랜 크롤링 → 마진 계산 → AI SEO 최적화 → 네이버 Commerce API 등록 / 엑셀 일괄 등록까지 풀 파이프라인. **화훼 아님** — "꽃/정원"은 브랜드 은유, 실제는 도매매 드롭십 셀렉트샵(자세한 오인 방지 사실은 `docs/DOMAIN_FACTS.md`).
...
- **Git**: commit/push 전 TSC 0 errors 확인. 커밋 메시지 prefix `feat:`/`fix:`/`docs:`/`refactor:`/`chore:`. 한글 다량 시 `.commit-msg.tmp`+`git commit -F`(#17). 저위험·즉시확인 가능한 변경은 main 직접 push, 화면 UI 재설계·여러 파일 묶음 작업은 feature 브랜치. **PR 리뷰·merge 승인은 항상 운영자 본인**(협업 체계, #320 — 에이전트가 늘어도 최종 merge 결정권은 운영자에게 있음). **push 직후 `scripts/verify-vercel-deploy.sh --wait` 의무**(#36, main push 시) — exit 1 시 webhook 진단.
- **브랜치·머지 리듬(#320, 2026-07-30 운영자 확정)**: 저녁 작업 세션 때 그날 만든 feature 브랜치를 **우선적으로 검토** — 문제없으면 그 자리에서 main에 merge한다. 그 외 시간에도 급하거나 중요한 판단이 필요하면 즉시 알린다. **브랜치를 며칠~몇 주 묵히지 않는다** — 오래 살아있는 브랜치일수록 그 사이 다른 작업과 충돌 위험이 커진다(업계 표준: 브랜치 수명은 짧게, 작은 단위로 자주 merge). push된 미merge 브랜치가 있으면 세션 브리핑(STEP 2)에 "며칠째 대기 중"인지 반드시 포함해 방치되지 않게 한다.
...
## 별도 개발 폴더 — extensions/ (절대 수정 금지)
- `extensions/kkotium-sourcing/` 은 대표님이 꽃틔움 작업용으로 웹앱과 **별도로** 개발 중인 크롬 확장(스크립트 도구)이다.
- 읽기·수정·삭제·커밋·.gitignore 추가 등 **어떤 조작도 하지 않는다**. `git add -A` 대신 대상 파일을 명시해 스테이징한다.
- 확인이 필요한 사항이 생기면 `docs/plan/EXTENSIONS_NOTES.md` 에 기록만 하고 대표님께 질문한다.
