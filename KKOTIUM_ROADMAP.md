...
> 최종 업데이트: 2026-04-14 (Phase C 전체 완료 — Phase D 중기 개선 계획 수립)
> **Phase A ✅ | Phase B ✅ | Phase C ✅ 전체 완료 | Phase D 중기 개선 대기**
...
| Task | 상태 | 내용 | 상세 |
|------|------|------|------|
| D-1 | ⬜ | 상품명 품질 체크 | 50자 제한 실시간 카운터, 금지키워드 감지 (셀러명/조사/수식어), 네이버 검색품질 경고, 씨앗심기+검색조련사 적용 |
| D-2 | ⬜ | 대시보드 위젯 레이아웃 정리 | GoodService/Profitability/MarketTrend 위젯 최적 배치, 모바일 반응형, 빠른 액션 바로가기 |
| D-3 | ⬜ | 경쟁 상품 모니터링 | 동일 카테고리 상위 키워드 패턴 추출, 가격/리뷰/순위 변화 Discord 알림, 네이버 쇼핑 API 활용 |
| D-4 | ⬜ | Naver DataLab API 직접 통합 | Perplexity 트렌드 대체, 카테고리별 실시간 트렌드 차트, 이미 ID 발급 완료 (F7Hga62gDOYxZ3KRtLTL) |
| D-5 | ⬜ | 씨앗 심기 탭 UX 추가 개선 | 탭별 완성도 뱃지 (체크/미완성 표시), 탭 전환 시 스크롤 위치 유지, 필수 항목 미입력 탭 강조 |
