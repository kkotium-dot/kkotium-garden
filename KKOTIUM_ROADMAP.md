...
> 확정 순서: E-7 → E-1 → E-3 → 나머지

| Task | 상태 | 내용 | 상세 |
|------|------|------|------|
| **E-7** | **✅** | **꼬띠 소싱 추천봇 (1단계)** | DataLab 트렌드 → 키워드 검색량 → 경쟁분석 → BlueOcean 점수 → Groq AI → Discord 매일 발송 + 대시보드 위젯 + 도매꾹/도매매 바로검색 |
| E-1 | ⬜ 대기 | 상세페이지 템플릿 빌더 | HTML 에디터 + 미리보기 + AEO Q&A 자동 배치 |
| E-3 | ⬜ 대기 | 상품 수명 주기 대시보드 | 등록일 대비 판매추이 + 좀비 시점 예측 |
| E-8 | ⬜ 대기 | 꼬띠 소싱 추천봇 (2단계) | 1단계 키워드로 도매꾹 OpenAPI 자동 검색 → 공급가/마진 자동 계산 → Discord 발송 |
| E-2 | ⬜ 대기 | 리뷰 관리 + 꼬띠 AI 자동 답변 | 네이버 리뷰 API 미제공 — 크롤링 또는 수동 |
| E-5 | ⬜ 대기 | 실제 매출 데이터 기반 꼬띠 추천 고도화 | 판매 패턴 학습 (상품 20~30개 이상 후) |
| E-4 | ⬜ 대기 | 반품안심케어 상태 관리 | 가입/수수료/보상금 추적 |
