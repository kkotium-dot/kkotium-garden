# KKOTIUM GARDEN — ROADMAP Archive 2026 Q2 (May)

> 본 파일은 ROADMAP.md에서 분할된 옛 인계 메시지 + Phase 이력 보관소입니다.
> 작업원칙 #31에 따라 ROADMAP.md가 1500줄 초과로 자동 분할 (2026-05-08).
> 
> **포함**: deprecated 인계 메시지 9개 + Phase A/B/C 완료 이력 + 작업 후보 표
> **상위 문서**: docs/plan/ROADMAP.md (현재 Sprint 6/7/8 계획 + 영구 참조)

---

## Section 1: Deprecated 인계 메시지 9개

# KKOTIUM GARDEN — 전체 작업 로드맵
> 최종 업데이트: 2026-05-08 (Z-3b 사이드바 보관함 deep-link 추가 ✅ — HUNT 섹션 "소싱 보관함" 메뉴(`/crawl?tab=history`) + computeActive 정밀화 + crawl 페이지 derived value 패턴(URL 단일 source) + dev hot-reload 충돌 학습 + 작업원칙 #26 일반화 2건 등록. 직전: Z-3d Phase A sub-graph cleanup commit `0e6df93`)
> ~~최종 업데이트 (구)~~: 2026-05-07 (Z-3d Phase A 잔재 sub-graph 일괄 정리 ✅ — 8개 파일 삭제 + dev 캐시 충돌 해소 + 작업원칙 #26 일반화 4건 등록. 직전: Z-3a CTA fix commit `3c329ba`)
...
> **Phase A ✅ | Phase B ✅ | Phase C ✅ | Phase D ✅ | Phase E (E-7, E-1, E-3, E-8) ✅ | Phase E+ Sprint 1·2·3·4·5 ✅ | Sprint 6 E-15 전체 완료 ✅ | 옵션 C/D/E Part 1 SWR 확장 ✅ | 워크플로우 재설계 Sprint A1a~A3-4a 완료 ✅ | Tailscale Funnel architecture ✅ | cron sync 통합 ✅ | 폴더 구조 정리 + 작업원칙 #29 강화 ✅ | UX/IA 마스터 블루프린트 v1 ✅ | Z-1/Z-2 진단 + Z-3a fix ✅ | Z-3d Phase A sub-graph cleanup ✅ | Z-3b 사이드바 보관함 deep-link ✅**
> **다음 작업: Z-3c' (/products/sourced 의미 명확화, 메인 워크플로우 4곳 영향) → Z-3e (백업 파일 49개 일괄 정리) → Z-Sec (RLS 정책) 순.** 자세한 인계 메시지는 `docs/plan/SESSION_LOG.md` "2026-05-08 세션 (Z-3b 사이드바 보관함 deep-link 추가)" 정독 + 답변 끝 인계 코드 블록 참조 (이전 Z-3 메시지는 deprecated)
...
> - **`src/app/products/page.tsx`** (-9 net, 1352 → 약 1330): `useState` 3개(raw/loading/error) + `useCallback` fetchProducts + 첫 `useEffect` trio 제거 + alias 트릭(`refresh: fetchProducts`)으로 L852/L907/L1314/L1335 호출처 0줄 변경. L915/L928/L943 `setRaw → setRawProducts` rename. UI/렌더 0 변경. 액션성 fetch 보존(register / shipping-templates / naver/excel / naver/products/sync / DELETE / PATCH × 3) — 쓰기라 SWR 대상 아님.
> - **Chrome MCP 라이브 검증 5항목 통과**: `/products` 8개 상품 정상 표시 / 회귀 `/dashboard` (`useProductsList` 2 호출처 정상) / 회귀 `/naver-seo` (A3-3a SWR도 정상) / HTTP `/products` 200 / `/api/products?limit=5` 200 (success/products/total 키 호환).
> - **이어받기 사례**: 직전 채팅에서 단계 1~5 완료 후 단계 6 직전 컨텍스트 한계 중단 → 본 세션에서 working tree 보존 확인(+72/-27 동일) 후 단계 6부터 이어서 진행. **작업원칙 (m) 강화 사례** — 이어받기 세션에서는 코드 재작성 절대 금지, git status 확인 후 중단 지점부터 이어서 진행.
> - 자세한 기록은 KKOTIUM_SESSION_LOG.md 최상단 "2026-05-05 세션 — 워크플로우 재설계 Sprint Part A3-3b 완료" 참조.

> **✨ A3-3a + A3-3b 페이지 SWR 확장 2/2 완료 ✅**: 검색 조련사 + 정원 창고 모두 SWR 마이그 완료. 훅 카운트 14개 (A3-2 13 + A3-3a +1 / A3-3b는 기존 훅 강화).

> **워크플로우 재설계 Sprint Part A3-4 작업 후보 (꽃졔님 선택 필요)**:
>
> | 후보 | 작업 내용 | 추정 분량 | 끊지마 관점 |
> |---|---|---|---|
> | **A** mascot SVG 위젯 강화 | 대시보드 4섹션 mascot pill UX 정리 + 꼬띠 페르소나(레드 튤립/카우걸 부츠) 일관성 검수 + 새 상황별 일러스트 추가 검토 | 작음 (1~2일) | 시각 강화, UX 계속성 |
> | **B** 한달리뷰 (E-2C 잔여) | 리뷰 보상 최적 설정 가이드 — 한달사용 리뷰 2단계 수집 구조 + 혜택 탭 UI | 중간 (UI 신설 + 가이드) | 새싹셀러 → 파워셀러 성장 핵심, 매출 임팩트 가장 큼 |
> | **C** NaverSeoProductTable row-level fetch SWR 마이그 | `market-analysis`, `keyword-stats` — 행별 동적 query라 useSWR 직접 사용 패턴 (Hook 추가 아닌 함수형 useSWR) | 작음~중간 (테이블 1개) | SWR 마이그 흐름 유지, A3-3 마무리 |
>
> **꽃졔님 새싹셀러 컨텍스트 추천**: **B (한달리뷰)** 가 매출 임팩트 가장 큼 — 파워셀러 조건(구매확정률 + 리뷰수 + 굿서비스) 중 리뷰수를 직접 끌어올리는 작업. SWR 흐름 유지를 원하면 **C (row-level fetch)**. 시각 강화면 **A (mascot)**.

> **기존 기능 0개 삭제** (작업원칙 27) — 선택하지 않은 후보 2개는 다음 Part로 보류. 이번 sprint는 1개 후보만 단독 진행.

> 아래 코드 블록을 그대로 복붙해서 사용.

```
꽃틔움 가든 개발 이어서 진행합니다. KKOTIUM_PROGRESS.md, KKOTIUM_ROADMAP.md, KKOTIUM_SESSION_LOG.md를 읽고
워크플로우 재설계 Sprint Part A3-4 작업을 시작해주세요. 다음 3개 후보 중 꽃졔님이 선택한 하나만 단독 진행 (동시 진행 금지):
  A) mascot SVG 위젯 강화 (대시보드 4섹션 mascot pill + 꼬띠 페르소나 일관성)
  B) 한달리뷰 (E-2C 잔여 — 리뷰 보상 최적 설정 가이드 + 혜택 탭 UI) — 추천 (새싹 → 파워셀러 성장 핵심)
  C) NaverSeoProductTable row-level fetch SWR 마이그 (행별 동적 query — useSWR 직접 사용 패턴)
...
1. (a) git rev-parse HEAD origin/main → 두 값 같은지 확인 (기준 HEAD는 본 세션 마무리 commit — A3-3b)
   (b) git --no-pager log --oneline -10 → 이번 메시지에 명시되지 않은 commit 있으면 읽고 대응
   ...
   (k) 작업원칙 27: 기존 기능 0개 삭제 — 위치 재배치 OK, 삭제/축소 0
   (l) 메모리 내 작업원칙 — heredoc 절대 금지
   (m) 이어받기 세션 사전 점검 강화 — 직전 채팅이 commit 직전 중단된 경우 git status로 working tree 보존 확인 후 코드 재작성 절대 금지(중복 작업 사고 위험) → 중단 지점부터 이어서 진행 (A3-3b 본 세션의 핵심 교훈)
   (n) deferred 도구 첫 호출 에러 응답 받아도 파일에 적용 가능 — namespace 중복 명령 절대 금지 + edit 후 즉시 raw 검증
   (o) MD 한글 직접 입력 — 정규화 절대 금지 (A3-3a 사고 교훈)
   (p) Claude Desktop MCP 4분 hang 패턴 — iterm/Filesystem 양쪽 모두 hang 시 재시도 금지 + Claude Desktop 재시작 요청 (A3-3b 본 세션 교훈)
2. KKOTIUM_PROGRESS.md 상단 헤더 + 꽃졔님 핵심 강조사항 섹션 정독
3. KKOTIUM_SESSION_LOG.md 최상단 "2026-05-05 세션 — 워크플로우 재설계 Sprint Part A3-3b 완료" 정독 (이어받기 교훈 포함)
4. 꽃졔님 선택한 후보(A/B/C)에 따른 대상 파일 정독 (A: src/components/dashboard/* + mascot 아이콘 / B: src/lib/review-care-* + 관련 UI / C: src/components/naver-seo/NaverSeoProductTable.tsx)
5. 작업 계획 브리핑 후 꽃졔님 승인 받고 시작

[A3-4 단계 권장 (후보별 세부는 승인 시 확정)]
- [단계 1] 선택된 후보의 현재 구조 grep + 수정 대상 라인 범위 확정
- [단계 2] 필요시 Hook/유틸/상수 신설 (제네릭 강화, error surface 패턴 적용)
- [단계 3] 대상 파일 마이그 또는 신규 구현
- [단계 4] TSC 0 errors 확인
- [단계 5] Chrome MCP 라이브 검증 (선택 후보별 명시 시나리오)
- [단계 6] PROGRESS/ROADMAP/SESSION_LOG 갱신 + commit + push 단일 라인
- [단계 7] 새 인계 메시지 작성 (A3-5 또는 다음 sprint 후보)

작업 분량 안전 마진:
- 선택 후보가 100줄+ 도달 예상 시 증분 git diff로 시각적 검증
...
## 🗂 Part A3-3b 메시지 (참고용 보존 — deprecated, 위 A3-4 메시지를 대신 사용)
...
> **A3-2 완료 ✅ (2026-05-05 본 세션, commit 본 세션 마무리 commit)**:
> - **`src/lib/hooks/useDashboardData.ts`** (+53줄, 13번째 훅): `useEventTimeline()` strict-typed (`ProductEventApiItem`) + `SWR_PROFILE_5MIN` cadence + `refresh()` 노출. 기존 12개 훅 패턴 그대로.
> - **`src/components/dashboard/EventTimeline.tsx`** (-12 +6 = 정합 -6줄): `useState`/`useEffect`/`load()` 제거 + `useEventTimeline` 훅 사용으로 교체. UI/렌더 0 변경. RefreshCw 버튼만 `onClick={refresh}`로 교체.
> - **Chrome MCP 라이브 검증 5항목 통과**: EventTimeline API 2건 = UI 2건 일치 / ConfirmationReminderWidget 회귀 / UploadReadiness 75점 회귀 / 4섹션 mascot pill 보존 / 자체 fetch 0개.
> - **API 라이브 회귀 점검**: `/api/events/recent` HTTP 200 + 2건 정확, `/api/orders/confirmation-pending` HTTP 200 + JSON 정확.
> - 자세한 기록은 KKOTIUM_SESSION_LOG.md 최상단 "2026-05-05 세션 — 워크플로우 재설계 Sprint Part A3-2 완료" 참조.

> **워크플로우 재설계 Sprint Part A3-3 작업 후보 (자체 판단 우선순위)**:
>
> | 우선순위 | 작업 | 컨텍스트 비용 | 근거 |
> |---|---|---|---|
> | **1번 (★ 추천)** | **다른 페이지 SWR 확장 (정원 창고 + 검색 조련사)** | 가변 (페이지별 분할 권장) | 자체 fetch 잔존 페이지들. A3-2에서 검증된 패턴(자체 fetch → useDashboardData 훅 + Strict typing) 그대로 재사용. 첫 단계: 정원 창고(`/products`) + 검색 조련사(`/keywords`) 자체 fetch 개수 + 의존성 분석 → 단독/묶음 결정. |
> | 2번 | mascot SVG 자산 통합 | 작음~중 (디자인 자산 필요) | accessory 텍스트(`watering_can`, `heart_gun`, `fountain_dance`) → 인라인 SVG. 꽃졔님 디자인 자산 입력 대기. |
> | 3번 | (보너스) 한달사용 리뷰 2단계 가이드 | 가변 | 리뷰 볼륨 2배 효과. A3-1b 패턴(D+30 윈도우)을 그대로 재사용 가능 — D+30~35 미작성 추적 + 알림톡 발송. |
>
> **자체 판단 추천**: A3-3 첫 채팅에서 **1번 다른 페이지 SWR 확장 — 정원 창고/검색 조련사 분석 + 단독/묶음 결정**. 그 후 A3-4에서 2·3번 결정.

> **A3-3 작업 범위 (1번 가정)**:
> 1. `src/app/products/page.tsx`(정원 창고) + `src/app/keywords/page.tsx`(검색 조련사) 자체 fetch 패턴 정독 — 개수 + 의존 데이터 + 사용자 액션 매핑 확인
> 2. 단독/묶음 결정 (자체 fetch 1~2개면 단독 OK, 3개+ 또는 의존성 있으면 페이지별 분할)
> 3. `src/lib/hooks/useDashboardData.ts`에 14번째+ 훅 추가 (SWR_PROFILE 적절 cadence 선택)
> 4. 페이지에서 자체 fetch 제거 + 훅 사용으로 교체 + refresh 노출
> 5. TSC 0 errors 확인 + Chrome MCP 라이브 검증 (페이지 정상 표시 + 다른 페이지/위젯 회귀 없음 + 75점 회귀)
> 6. PROGRESS/ROADMAP/SESSION_LOG 갱신 + commit + push 단일 라인
> 7. 새 인계 메시지 작성 (A3-4 후보 — mascot SVG / 한달리뷰 가이드)

> **기존 기능 0개 삭제** (작업원칙 27) — 13개 대시보드 위젯 + 4섹션 + 모드 토글 + KkottiBriefing + 4섹션 mascot pill + 동적 subtitle + ModeActionHint + Section 3 모드별 정렬 + 구매확정 리마인더 위젯 + EventTimeline 모두 보존, 작업이 SWR 마이그면 오직 fetch 방식만 교체

> 아래 코드 블록을 그대로 복붙해서 사용.

```
꽃틔움 가든 개발 이어서 진행합니다. KKOTIUM_PROGRESS.md, KKOTIUM_ROADMAP.md, KKOTIUM_SESSION_LOG.md를 읽고
워크플로우 재설계 Sprint Part A3-3 작업을 시작해주세요.
...
작업 시작 전 필수 (작업원칙 21+22+23+24+25+26+27 적용):
1. (a) git rev-parse HEAD origin/main → 두 값 같은지 확인 (기준 HEAD는 본 세션 마무리 commit — A3-2)
   (b) git --no-pager log --oneline -10 → 이번 메시지에 명시되지 않은 commit 있으면 읽고 대응
   (c) git status 깨끗한지 확인 (dirty면 검토 후 처리 — 덮어쓰기 절대 금지)
   (d) lsof -i :3000 또는 curl http://localhost:3000 → dev 서버 상태 확인
   (e) 이 메시지의 가정과 실제가 다르면 즉시 정직 보고 후 재분석
   ...
   (l) 메모리 내 작업원칙 — heredoc 절대 금지
   (m) 이어받기 세션 사전 점검 강화 — 직전 채팅이 commit 직전 중단된 경우 raw 검증으로 일치 확인 후 진입
2. KKOTIUM_PROGRESS.md "2026-05-05 세션 요약 — 워크플로우 재설계 Sprint Part A3-2 완료" 정독
3. KKOTIUM_SESSION_LOG.md 최상단 동일 세션 정독
4. 1번 다른 페이지 SWR 확장을 진행할 경우: `src/app/products/page.tsx`(정원 창고) + `src/app/keywords/page.tsx`(검색 조련사) 자체 fetch 패턴 grep + `src/lib/hooks/useDashboardData.ts`의 기존 13개 훅 패턴 정독
5. 작업 계획 브리핑 후 꽃졔님 승인 받고 시작

[A3-3 단계 권장 — 1번 가정]
- [단계 1] 정원 창고 + 검색 조련사 자체 fetch 위치/개수/의존성 분석
- [단계 2] 단독/묶음 결정 (자체 fetch 1~2개면 단독 OK, 3개+ 또는 의존성 있으면 페이지별 분할)
- [단계 3] useDashboardData.ts에 14번째+ 훅 추가
- [단계 4] 페이지에서 자체 fetch 제거 + 훅 + refresh 사용으로 교체
- [단계 5] TSC 0 errors 확인 + Chrome MCP 라이브 검증
- [단계 6] PROGRESS/ROADMAP/SESSION_LOG 갱신 + commit + push 단일 라인
- [단계 7] 새 인계 메시지 작성 (A3-4 후보)

작업 분량 안전 마진:
- 단계 1 분석에서 페이지당 자체 fetch 3개+ 발견 시 즉시 페이지 분할 (A3-3a 정원 창고 / A3-3b 검색 조련사)
- 단계 5 검증은 컨텍스트 여유 있을 때만 진행
- 세션별 자세한 기록은 KKOTIUM_SESSION_LOG.md에 작성, PROGRESS.md/ROADMAP.md는 핵심 요약만 유지
...
## 🗂 Part A3-2 메시지 (참고용 보존 — deprecated, 위 A3-3 메시지를 대신 사용)

## 🎯 다음 새 채팅 시작 메시지 (워크플로우 재설계 Sprint Part A3-2 — 2026-05-05 작성)

> **A3-1b UI 완료 ✅ (2026-05-05 본 세션, commit 본 세션 마무리 commit)**:
...
> - **`src/app/dashboard/page.tsx`** (+2줄): L28 import + L499 Section 2 (action) 끝에 `mode === 'today'` 조건부 배치.
> - **Chrome MCP 라이브 검증 5항목 통과**: 위젯 표시 / Solapi 미입력 안내 + 진행률 0% / DRAFT 8개 평균 75점 회귀 / 4섹션 mascot pill (^_^/^ㅂ^×2/✿ㅅ✿) 보존 / Section 3 모드별 정렬 + 동적 subtitle + ModeActionHint 보존.
> - **API 라이브 회귀 점검**: `/api/orders/confirmation-pending` HTTP 200 + JSON 정확.
> - 자세한 기록은 KKOTIUM_SESSION_LOG.md 최상단 "2026-05-05 세션 — 워크플로우 재설계 Sprint Part A3-1b UI 완료" 참조.

> **워크플로우 재설계 Sprint Part A3-2 작업 후보 (자체 판단 우선순위)**:
>
> | 우선순위 | 작업 | 컨텍스트 비용 | 근거 |
> |---|---|---|---|
> | **1번 (★ 추천)** | **EventTimeline SWR 마이그** | 작음 (단독 안전) | 자체 fetch 패턴 → `useEventTimeline()` 5분 cadence 신설. 기존 11개 SWR 훅 패턴 그대로 답습 → 빠른 마무리. A3-1b 직후 가벼운 정착 작업으로 적합. |
> | 2번 | 다른 페이지 SWR 확장 (정원 창고 / 검색 조련사) | 가변 (페이지별 분할 권장) | 자체 fetch가 아직 남아있는 페이지들. 페이지별로 컨텍스트 비용 다르므로 첫 분석 후 분할 결정. |
> | 3번 | mascot SVG 자산 통합 | 작음~중 (디자인 자산 필요) | accessory 텍스트(`watering_can`, `heart_gun`, `fountain_dance`) → 인라인 SVG. 꽃졔님 디자인 자산 입력 대기. |
> | 4번 | (보너스) 한달사용 리뷰 2단계 가이드 | 가변 | 리뷰 볼륨 2배 효과. A3-1b 패턴(D+30 윈도우)을 그대로 재사용 가능 — D+30~35 미작성 추적 + 알림톡 발송. |
>
> **자체 판단 추천**: A3-2 첫 채팅에서 **1번 EventTimeline SWR 단독 진행**. 그 후 A3-3에서 2번 묶음, A3-4에서 3·4번 결정.

> **A3-2 작업 범위 (1번 EventTimeline SWR 마이그 가정)**:
> 1. `src/components/dashboard/EventTimelineWidget.tsx` (또는 동등 위치) 정독 — 자체 fetch 패턴 확인
> 2. `src/lib/hooks/useDashboardData.ts`에 `useEventTimeline()` 훅 추가 (SWR_PROFILE_5MIN 또는 적절 cadence)
> 3. EventTimelineWidget.tsx에서 자체 fetch 제거 + 훅 사용으로 교체 + refresh 노출
> 4. Chrome MCP 라이브 검증: EventTimeline 정상 표시 + 다른 위젯 회귀 없음 + 75점 회귀
> 5. PROGRESS/ROADMAP/SESSION_LOG 갱신 + commit + push 단일 라인
> 6. 새 인계 메시지 작성 (A3-3 후보 — 다른 페이지 SWR 확장 / mascot SVG / 한달리뷰 가이드)

> **기존 기능 0개 삭제** (작업원칙 27) — 13개 위젯 + 4섹션 + 모드 토글 + KkottiBriefing + 4섹션 mascot pill + 동적 subtitle + ModeActionHint + Section 3 모드별 정렬 + 구매확정 리마인더 위젯 모두 보존, 작업이 SWR 마이그면 오직 fetch 방식만 교체

> 아래 코드 블록을 그대로 복붙해서 사용.

```
꽃틔움 가든 개발 이어서 진행합니다. KKOTIUM_PROGRESS.md, KKOTIUM_ROADMAP.md, KKOTIUM_SESSION_LOG.md를 읽고
워크플로우 재설계 Sprint Part A3-2 작업을 시작해주세요.
...
작업 시작 전 필수 (작업원칙 21+22+23+24+25+26+27 적용):
1. (a) git rev-parse HEAD origin/main → 두 값 같은지 확인 (기준 HEAD는 본 세션 마무리 commit — A3-1b UI)
   (b) git --no-pager log --oneline -10 → 이번 메시지에 명시되지 않은 commit 있으면 읽고 대응
   (c) git status 깨끗한지 확인 (dirty면 검토 후 처리 — 덮어쓰기 절대 금지)
   (d) lsof -i :3000 또는 curl http://localhost:3000 → dev 서버 상태 확인
   (e) 이 메시지의 가정과 실제가 다르면 즉시 정직 보고 후 재분석
   (f) 본 세션 commit은 그 turn 안에서 push까지 한 줄로 완료 (단일 라인 commit 메시지)
   (g) edit_file에서 한글 매칭 실패 시 Python 수동 NFC 정규화 절대 금지 → git restore + write_file 한글 직접 입력
   (h) edit_file 에러 응답 받아도 파일에 일부 적용될 수 있음 → head/grep/xxd로 raw 검증 우선
   (i) 문제 분석은 항상 (a) 즉각 원인 (b) 일반화 원인 두 단계로
   (j) 브라우저 테스트는 API 200 응답으로 대체 불가 — Chrome MCP로 실제 화면/숫자/동작 검증 필수
   ...
   (m) 이어받기 세션 사전 점검 강화 — 직전 채팅이 commit 직전 중단된 경우 보고된 상태와 실제 working tree 상태를 raw 검증으로 일치 확인 후 진입 (A3-1b 이어받기에서 일반화)
2. KKOTIUM_PROGRESS.md "2026-05-05 세션 요약 — 워크플로우 재설계 Sprint Part A3-1b 완료" 정독
3. KKOTIUM_SESSION_LOG.md 최상단 동일 세션 정독
4. 1번 EventTimeline SWR 마이그를 진행할 경우: `src/components/dashboard/EventTimelineWidget.tsx`(또는 동등 위치) 정독 + `src/lib/hooks/useDashboardData.ts`의 기존 11~12개 SWR 훅 패턴 정독
5. 작업 계획 브리핑 후 꽃졔님 승인 받고 시작

[A3-2 단계 권장 — 1번 EventTimeline SWR 가정]
- [단계 1] EventTimelineWidget.tsx 자체 fetch 패턴 정독 + useDashboardData.ts 훅 패턴 정독
- [단계 2] useEventTimeline() 훅 추가 (SWR_PROFILE_5MIN 또는 적절 cadence)
- [단계 3] EventTimelineWidget.tsx에서 자체 fetch 제거 + 훅 + refresh 사용으로 교체
- [단계 4] TSC 0 errors 확인 + Chrome MCP 라이브 검증 (EventTimeline 정상 + 위젯 회귀 없음 + 75점 회귀)
- [단계 5] PROGRESS/ROADMAP/SESSION_LOG 갱신 + commit + push 단일 라인
- [단계 6] 새 인계 메시지 작성 (A3-3 후보)

작업 분량 안전 마진:
- 단계 1~2가 컨텍스트 한계 60% 도달 시 즉시 중간 commit + 새 채팅으로 분할 (A3-2a / A3-2b)
- 단계 4 검증은 컨텍스트 여유 있을 때만 진행
- 세션별 자세한 기록은 KKOTIUM_SESSION_LOG.md에 작성, PROGRESS.md/ROADMAP.md는 핵심 요약만 유지

답변 마지막에 복붙용 인계 메시지를 코드 블록으로 항상 명시 (꽃졔님 직접 지시 — 2026-05-04)
```
...
## Section 2: Phase A/B/C 완료 이력 + 작업 후보 표
