# PRINCIPLES_LEARNED — 작업원칙 아카이브 (~#227, 2026-07-30 분할)

> 이 파일은 `docs/plan/PRINCIPLES_LEARNED.md`가 1483줄로 1500줄 임계(#31)에 근접해 분할한 것입니다. **삭제된 내용은 없습니다** — 원칙 #46~#227 구간(이전 분할 rev와 동일한 방식) 전문을 그대로 이 파일로 이동했습니다. 현행 파일은 #254 이후를 유지합니다.

> 이전 분할: `docs/plan/archive/PRINCIPLES_LEARNED_archived-2026-07-22.md`(#1~#45 구간, PRINCIPLES_CODE.md로 이관된 것과는 별개 — 그쪽은 다른 이관 이력).
