...
## ⚠️ 미이관 구간 인덱스 — 원칙 #149~#253 (2026-07-14 Desktop 발견, 미해결 문서부채)

**문제**: 작업원칙 #31(b)는 학습된 패턴을 PRINCIPLES_LEARNED.md로 분리 이관하도록 규정하나, **#149~#253 사이 원칙들은 정식 이관되지 않고 `docs/plan/PARALLEL_WORK_TRACKER.md`의 압축 로그 안에 인라인(`원칙 #NNN 박제`)으로만 존재**한다. 본 파일에는 #148 다음 바로 #254로 뛴다 — 약 100개 원칙 본문이 이 파일에 없다.

**발견 경위**: 2026-07-14 Desktop 세션에서 `docs/plan/PARALLEL_WORK_TRACKER.md`가 실제로는 2026-06-24(rev50)에서 멈춰 있고(07-11~07-13의 P1~P4 IA재설계·페르소나 작업이 전혀 미반영), `TASK_BRIDGE.md`도 2026-05-26 이후 갱신 중단을 확인하는 과정에서 원칙 인덱스 갭이 함께 드러남.

**확인된 번호 범위(트래커 grep 실측, 2026-07-14)**: #149~#156, #165, #181~#203, #216, #221~#227, #231 — 각 항목의 규칙 본문은 `docs/plan/PARALLEL_WORK_TRACKER.md`의 해당 rev 블록 안에 "원칙 #NNN 박제: ..." 형태로 존재.

**다음 세션(Code) 필수 작업**:
1. `docs/plan/PARALLEL_WORK_TRACKER.md`를 rev50(2026-06-24)부터 최신까지 순서대로 훑어 원칙 #149~#253 각각의 규칙 본문·근거·검증 사례를 추출
2. 본 파일에 번호순으로 정식 절 추가(기존 항목과 동일 포맷: 제목/맥락/규칙/근거)
3. 이관 완료 후 PARALLEL_WORK_TRACKER 상단에 "#149~#253 원칙 전문은 PRINCIPLES_LEARNED.md 참조"로 역참조 추가
4. 완료 시 본 경고 블록 삭제

**당장 급하지 않은 이유**: 원칙 번호는 코드 커밋 메시지·핸드오프 문서에 참조로만 쓰이고 있어 당장 앱 동작에 영향은 없음. 그러나 방치 시 다음 세션이 같은 실수를 반복할 위험(예: #196/#197 네이버 full-replace PUT 규칙, #71 사실성 레인 등 중요 안전 규칙 다수가 이 구간에 포함).
