...
1인 개발 + AI 페어 프로그래밍 환경에서 feature 브랜치를 언제 main에 합칠지에 대한 운영자 확정 규칙.
...
- 저위험(문서만, 소규모 문구수정 등)은 main 직접 push, 화면 재설계·다중파일 묶음은 feature 브랜치 — 1인 개발이므로 PR 리뷰어 없이 운영자 본인 검토 후 직접 merge.
