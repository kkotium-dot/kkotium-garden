# 꽃틔움 가든 — 병행작업 트래커 (누락 0 원칙) · 최종 업데이트 2026-06-24 (rev40 · P1-f products/new+Tower fixes 완료(#149·#132)[신규 공용 프리미티브 ElevatedDropdown(D4 OverflowMenu 포털 메커니즘 일반화·createPortal(body)+fixed+z9999+rect동기·scroll(capture)/resize/ResizeObserver·뷰포트 하단 flip-up·anchor+panel 인지 outside-close)·F1 플랫폼/공급사 드롭다운 포털화(USection overflow:hidden 탈출·각 picker wrapRef outside-click 제거→프리미티브로)·F4(i) Tower 실전마진 카테고리 검색 포털화(우레일 클립 탈출)+(ii) 우레일 overflowX:hidden+가로패딩6(overflowY:auto가 overflow-x도 auto로 계산→flush 카드 포커스링 사이드클립 해소·가로스크롤바 0·세로유지)·추가 인라인 콤보 2종 포털화(SEO탭 카테고리검색·원산지 — onBlur close+onMouseDown preventDefault 유지)·F2 기본정보 순서: 대체상품 관리 LAST(가격→플랫폼·공급사→상품코드→브랜드·원산지·수입사→옵션→대체상품)·F3 상단 저장바 justify-end(우측정렬)·폰트 후속(#132): 운영자 발견=USection 카드타이틀 prod Pretendard → 결정=카드타이틀 Pretendard 유지(라우트 히어로만 브랜드 디스플레이·tasteful #147)·USection 스팬 불필요 font-display 제거]·e9da562·tsc0·build0·preview(desktop 1440+mobile) 검증: 플랫폼 드롭다운 isBodyChild+fixed+z9999·폭 anchor 일치(310=310)·클립0·도매매 선택 커밋됨(outside-close에 안삼켜짐)+#137 관리 케밥 표면화(F1이 #137 언블록)·Tower 카테고리검색 포털(width407·결과19)·옵션 Y<대체상품 Y(1688<2053)·저장바 flex-end·콘솔 에러0(기존 SVG ellipse hydration warn만)·#135 edit 공유·SD-01 무접촉·양배포 verify OK(prod e9da562)·prod smoke products/new+dashboard 200·Desktop 라이브 검증대기(#45/#88·VERIFICATION PLAN F1 케밥 portal/F4 클립/F2 순서/F3 우측/narrow 1024·375 운영자 spot-check) · prev rev39 · FONT #132 완료(브랜드 디스플레이 폰트 하이브리드)[Cafe24 Ssurround 셀프호스트 next/font/local·--font-brand 변수(--font-display는 상세 preset 렌더러 Noto Serif KR 점유→신규 토큰 분리·발행물 회귀0)·font-display:swap·Tailwind fontFamily.display→[var(--font-brand),Pretendard]·CLS폴백·woff2 199KB]·적용=제목만(라우트히어로 정원일지/정원창고/꿀통꽃나들이/씨앗심기·gp-display/h1/h2 시맨틱헤딩[studio·workbench·seo드로어]·USection 카드타이틀 단일컴포넌트·모달타이틀)·유지=본문/폼/표/버튼/뱃지/숫자데이터 전부 Pretendard tabular-nums(USection meta 포함)·보류(Desktop 시각승인·#147/#73 오버플로·과밀)=Latin KKOTIUM 워드마크(Arial Black 로고)/sidebar nav rows(14px ellipsis)/step라벨/헤더계정라벨·e5b6493·tsc0·build0·양배포 verify OK(prod e5b6493)·prod smoke: dashboard 200·woff2 200 font/woff2(/_next/static/media/...-s.p.woff2 head preload)·heading computed=Cafe24(document.fonts loaded)·body/숫자=Pretendard·~382px 오버플로0·SD-01 무접촉·롤백=fontFamily.display 토큰+--font-brand var 제거·Desktop 실렌더 검증대기(#45/#88)·#146 큐(다음·독립)·CR-1 승인대기 · prev rev38 · S2-B.2 step-sync 폴리시 완료[배양실 상단 신호등 StepStatus 스트립(썸네일·상세·SEO·발행=미작업/진행/완료·공용 StatusBadge tone neutral/warning/success·기존 actions/engine 상태만·새 점수로직 0·#55)+REVEAL(스트립=step nav·클릭 시 해당 step 폼만 노출·active 하이라이트·kk-step-reveal 페이드)+꼬띠 마이크로카피 센터 버블 렌더 확인·i18n stepStatus(#35)·폰트 보류 비차단]·7a95368·tsc0/clean build0/preview 실DOM(SEO=완료 라이브 DNA·발행 클릭 시 publish 폼만 노출·콘솔0) · 노트2 해소(코드변경0): (a) aside 밖 w:0 깔끔형=AtelierShell 모바일 lg:hidden 사본(의도된 dual-tree #140·스트레이 더블렌더 아님) (b) 센터 꼬띠 버블 렌더 확인(w:287×38) · S2-B.1 Desktop PASS · prev rev37 · S2-B.1 Studio 중앙 클렌징 완료(relocation skeleton·#138)[무거운 단계 입력/생성 카드(Firefly/무드→카메라/AI진단/상세템플릿/배경선택=썸네일·상세·SEO·발행 StepGroup)를 중앙 캔버스 → 좌 배양실(cultivation) 패널로 JSX reparent·핸들러/상태/바인딩 무접촉(페이지 actions/engine/step 그대로)·step gating으로 step-sync 내재 / 중앙=제품헤더+꼬띠 가이드버블(현재 step)+라이브 미리보기(WorkbenchCanvas)+조립슬롯 정적 스텁(대표/추가/상세·충전로직 0=S2-D)·KkottiGuide StepGroup→중앙 단일화·i18n atelier.assembly 추가(#35)·#144 유지·SD-01 무접촉]·559a284 · tsc0·build0·preview 실DOM 검증(배양실=썸네일 4변형 카드·중앙=꼬띠+미리보기+조립슬롯·단계폼 중앙 부재·콘솔 0) · P1-a refine Desktop PASS(#137 갭: 플랫폼/공급사만 적용·대체상품/옵션/asset 행=자식컴포넌트라 미적용·후속) · 원칙 #148 박제(전역 chrome/transient infra vs 기능 구분 검증) · prev rev36 · P1-a refine ③통일형 완료(2단계·#138): P1-a.1 USection 정본카드 신설[아이콘+타이틀·번호뱃지0·강제 아코디언0·중립보더 #E4DFD4·핑크=액티브/CTA/필수만·tabular-nums] + RSection/DSection→USection 위임(전 ~15섹션 4탭 통일·아이콘 중립)·a9f917a / P1-a.2 [2 기본정보] 재배치(가격→플랫폼·공급사→상품코드→브랜드·원산지·수입사→대체상품→옵션 LAST·번들 카드 분할·basic fragment 2→1) + #137 케밥(플랫폼/공급사 수정·삭제→공용 포털 OverflowMenu) + 탭3·4 USection 자동상속·fc8d206 · 스킨/구조/재배치만(#132)·필드/핸들러/검증/바인딩 무접촉·#135 edit 자동전파·tsc0·clean build0(dev/.next 충돌 1회→클린 재빌드 통과)·preview 실DOM 검증(탭1·2 통일카드·탭2 순서 실측·번호뱃지0·콘솔=기존 SVG warn만) · P1-e/P1-d Desktop PASS 표기 · S2-B 중앙 클렌징=병렬 활성 레인(독립·/studio·운영자 결정 불요·#124 무충돌·#132) · prev rev35 · P1-e PLANT Tower 단순화 완료[히어로 2지표·SEO 신호칩5종·TowerSection 접이식 래퍼 제거(#147 과빌드정리)·꿀통지수→SEO 검색최적화 점수 최상단 평평·기능보존(꿀통/SEO/실전마진/마진어드/AI SEO/준비도/엑셀)·바인딩보존(판매가→실마진·꿀통, 검색최적화→SEO점수)·상단 sticky 저장바(임시/DB/네이버 only)·저장후 온실아틀리에=이미지탭 전용 CTA·순감 -131줄·3474d3d] + P1-d CRAWL-GRID refine 완료[D1 헤더 2행 정렬(타이틀+필터뱃지 / 공급사+검색+초기화·edge)·D2 밀도토글 제거(rowPadY 9)·D3 도매가/예상마진율 헤더 우측정렬(justifySelf:end)+마진셀 우측·D5 일괄등록바 fixed 하단 플로팅(선택시·해제시 사라짐)·D4 #62 OverflowMenu 포털화(createPortal fixed·overflow 클리핑/z-index 해소·전역)·1e65c20] · tsc0·build0·양배포 verify OK(prod 1e65c20) · P1-a refine(블록통일·브랜드>옵션·#137 케밥)=운영자 그린라이트 대기(#147 디자인결정 위임) · CR-1(예상마진율 ~13% 군집·40%강조 미발화)=백로그 · 원칙 #147 박제 · prev rev33 · P1-c CRAWL-GRID-v2 완료[/crawl 이력 카드→고밀도 표 Grid 5컬럼·현황박스 제거→필터탭 뱃지(실측 전체5/소싱2/대기1/완료2)·컨텍스트 단일 스마트액션(씨앗심기/이어서작업/스토어보기)·예상마진율 미니게이지+40%강조·마진 DESC 기본정렬+정렬가능(실측)·밀도토글·sticky 일괄바·키보드내비·#62 StatusPill별칭/--border-neutral/tabular-nums·로직 무변경#132·preview 실데이터 e2e·콘솔0] + P1-b Desktop e2e PASS·바인딩 실측(판매가12000/공급가5000→실마진율 25.6%) · #34 정리(백업5건#43) · tsc0 · prev rev32 · P1-b TOWER-refine 완료[우패널 구조 격상·기능0삭제#132 — 히어로 2지표 상단고정(SEO 상위노출확률+게이지 / 실마진율 calcNetMargin+게이지·실측 25.6%) + 네이버우선 SEO 신호칩5종 실시간 + 진단패널 통일 접이식(TowerSection·6패널 verbatim 보존) · 좌패널/탭/state 무접촉 · preview e2e+바인딩 실측] + P1-a Desktop e2e PASS · #135 edit 패리티 실측(?edit 실상품 4탭+Tower+prefill OK) · #34 정리(백업/중복 5건 git rm·#43 CLEAN) · tsc0 · prev rev31 · P1-a PLANT-IA-v2 4탭 재구성 완료[5탭→4탭(검색최적화 FRONT·기본정보·이미지·배송정책)·옵션탭=기본정보로 폴드·카테고리+상품명 검색최적화 탭 이동·상품명 단일화(readOnly 미러 제거, 1개 편집필드)·브랜드/원산지/수입사+대체상품 배송→기본정보(D-ATTR-MOVE)·배송 DSection 펼침(POLICY UNCOLLAPSE)·단일 고정 액션바·#135 create/edit 공유(edit route=redirect→?edit)·Tower 무접촉·tsc0·preview 4탭 멤버십 e2e검증(누수0)] + S2-A.2 Desktop e2e PASS(①②③④) · prev rev30 · S2-A.2 ④ D-BORDER 완료[구조/폼 기본보더=크림파생 웜그레이 중립토큰 --border-neutral(#E4DFD4)/-strong(#D2CBBC)·전역 .border-gray/.border-stone sweep(globals 85-87) + 폼등록 .border-stone-200 !important(161)을 --color-border(핑크)→중립 재지정·브랜드핑크는 accent/card(.kk-card·--card-border·badge·SourcingRecommend카드=--color-border)만 유지·tsc0·배포후 Desktop e2e] · prev rev29 · S2-A.1 Desktop e2e PASS + S2-A.2 브랜드 폴리시[StatusBadge 의미중립 토큰(미적용/비활성 핑크화 방지)·SourcingRecommend 인라인보더→토큰·에셋카드 헤더 #144 하드닝] · 발견 3건(중립뱃지·인라인보더 사각지대·헤더 미세갭) · 원칙 #146 박제 · prev rev28 · S2-A Desktop prod e2e PASS[68cb6e1·이중사이드바 탭/접힘·토큰·#144 active·4페이지 회귀0·#45] + S2-A.1 브랜드 리스킨[Tailwind pink팔레트→브랜드토큰 config 재정의·preflight borderColor.DEFAULT→브랜드·회색보더 전역0·바hex핑크0·#145] · 원칙 #145 박제 · prev rev27 · 스튜디오 Stage2 S2-A 완료[AtelierShell flex-fill·calc매직넘버제거(#141)·이중사이드바 골격(창고/배양실/일지·접힘)·색토큰 중앙클렌징(--brand-red/--pink-soft/--cream·#142)·반응형4종 하드닝(#144)·STUDIO_UI_UX_GUIDELINES 신설] · 글로벌 main bounded(전4페이지 가로overflow0 셀프검증) · tsc0 · 원칙 #142/#143/#144 박제 · prev rev26 · 스튜디오 Stage1 구조완료[독립패널스크롤·에셋타일 컴포넌트·스테퍼 우측게이팅·#3 PASS] + lg:hidden 인라인 display 핫픽스[이중렌더 제거·prod결함·#62] · dev정지 후 build0·tsc0·커밋우선 #139 · 잔존 세로오버플로·#2·#4 → Stage2 편입 · 원칙 #140/#141 박제)
---
...
| P1-a PLANT-IA-v2 | /products/new 5탭→4탭 IA 재구성(create/edit 공유) | P1 | S2-A.2·794930f | ✅ DONE·**Desktop e2e PASS**·**#135 edit 패리티 실측**(`?edit=실상품id` → 4탭+Tower 렌더·상품명 prefill OK·에러0) — 권위 docs/design/PLANT_CRAWL_TOWER_REDESIGN_2026-06-24.md §1. 5탭(기본/옵션/이미지/검색최적화/배송)→**4탭 [검색최적화 FRONT·기본정보·이미지·배송정책]**. 변경: ①탭바 4개+기본탭=검색최적화(default) ②카테고리 RSection + 상품명(편집)을 검색최적화 탭으로 이동 ③상품명 단일화=readOnly 미러 삭제(편집필드 1개) ④옵션 RSection을 기본정보 탭에 폴드 ⑤브랜드/원산지/수입사 DSection + 대체상품 패널을 배송→기본정보 이동(D-ATTR-MOVE) ⑥배송 DSection 기본 펼침(useState(true)·POLICY UNCOLLAPSE) ⑦단일 고정 액션바(임시저장·DB저장·저장후 온실아틀리에·케밥) 유지 ⑧tabDone 재정의(옵션완성도→기본, 카테고리+상품명+키워드→검색최적화). #135: edit route는 `?edit` redirect=동일 컴포넌트 공유(구조 자동 전파). Tower(마진/SEO워크플로 우패널) 무접촉. preview 검증: 4탭 순서·기본탭·탭별 멤버십(검색=카테고리+상품명+SEO / 기본=기본정보+옵션+브랜드원산지+대체 / 배송=6 DSection 펼침)·누수0·콘솔 blocking 0(SVG ellipse hydration warn은 기존 무해). 후속(P1-a refine): 블록통일(RSection/DSection 단일 카드패턴)·탭내 순서(브랜드를 옵션 앞)·#137 케밥 정리 |
| P1-b TOWER-refine | /products/new 우패널 구조 격상(기능 0삭제·#132) | P1 | P1-a·fb7fd8d | ✅ DONE·**Desktop e2e PASS**(바인딩 실측 25.6%)·검증요청 — 권위 §2. 기능 0삭제, 구조만 격상: ①**히어로 2지표 상단 고정**(SEO 상위노출확률=`seoResult.score` 0-100+게이지+가장약한항목 한줄 / 실마진율=`calcNetMargin(공급가,판매가,배송비,수수료율,반품2%)` %+게이지·판매가/공급가 0이면 "입력 시 계산" 플레이스홀더). 수수료율=`getNaverFeeRate(categoryId)`(fallback 0.057). ②**네이버우선 SEO 신호칩 5종 실시간**(상품명20-50자·브랜드포함·셀러태그n/10·카테고리매칭·키워드밀도, pass=success/fail=neutral StatusBadge). ③**진단 패널 통일 접이식**(`TowerSection` 헬퍼=neutral보더·tabular-nums·접힘): 꿀통지수·마진어드바이저(d1&&d3 가드 보존)·실전마진계산·AI SEO분석·SEO점수상세·엑셀매핑체크리스트 — **6패널 props/콜백 verbatim 보존**(0삭제). **바인딩 실측**: 판매가12000·공급가5000 입력 → 실마진율 25.6% 라이브 산출(=(12000-5000-3000-684-240)/12000), 플레이스홀더 해제. edit `?edit` 동일전파. 우패널만 변경(좌패널/탭/state/handler 무접촉). 콘솔 blocking 0(기존 SVG ellipse warn만). 후속: P1-a refine(블록통일 등) 차기 빌드 |
| P1-c CRAWL-GRID-v2 | /crawl 이력 카드형 → 고밀도 SaaS 표 Grid | P1 | f887a0e(독립) | ✅ DONE·**Desktop PASS**(권위 §STATUS line11)·tsc0·preview e2e 실데이터 — 권위 §3. 로직 무변경(#132). ①상단 대형 현황박스 제거→**필터탭 뱃지 카운트**(전체5·소싱완료2·등록대기1·등록완료2·보류0·단건5·대량0 실측) ②카드→**표 Grid 5컬럼** [체크박스│상품정보(썸네일+이름+재고/공급사 muted 하단+태그)│도매가│예상마진율+미니게이지│스마트액션] ③**컨텍스트 단일 스마트액션**(SOURCED=검정[▶씨앗심기]=기존 등록시작 prefill verbatim / PENDING=빨강[이어서 작업]=product_id면 edit·아니면 prefill / REGISTERED=옅은회색[스토어 보기]) — 큰 빨강/초록 반복버튼 제거·완료표시/재분류/수정/원본/삭제는 OverflowMenu로 격하 ④예상마진율=calcNetMargin(도매가, calcPrefillSalePrice, 배송비, 0.057, 2)·**40%+ 강조**(굵은초록) ⑤**마진 DESC 기본정렬**(13.3→12.9 실측)+정렬가능 컬럼(도매가/마진 헤더 클릭 ▲▼·실측 도매가 24300→7980) ⑥밀도토글(조밀/여유) ⑦선택 시 sticky 일괄바 ⑧키보드내비(j/k/Enter·data-crawl-row/data-smart-action DOM구동·핸들러 무복제). #62 재사용: 공용 StatusBadge **as StatusPill 별칭**(로컬 StatusBadge line164 무접촉)·`--border-neutral`·tabular-nums. preview: 5행 렌더·콘솔 error 0. 후속(deferred): 행 호버 퀵프리뷰 팝오버(overflow:hidden 클리핑 리스크로 보류, 호버 하이라이트는 적용) |
| P1-e PLANT Tower 단순화 | /products/new 우패널 과빌드 정리 + 상단 저장바 (#132·#135·#147) | P1 | P1-b·baa8a3c | ✅ DONE·**Desktop PASS**(tsc0·build0·prod 3474d3d) — 권위 §P1-e. P1-b 과빌드 되돌림(#147): **E1** 히어로 2지표 블록 제거(꿀통지수+SEO 점수가 이미 제공·중복) **E2** TowerSection 접이식 래퍼 제거→패널 네이티브 구조 평평(동일타이틀 이중접힘 해소)·TowerSection 함수+ChevronDown import 제거 **E3** SEO 신호칩 5종 제거(SEO 점수상세 체크리스트와 중복) **E4** SEO 검색최적화 점수=꿀통지수 바로 아래(둘 최상단 at-a-glance) **E5** 전 기능 평평 보존(꿀통지수/SEO 점수/실전 마진/마진 어드바이저/AI SEO/업로드 준비도/엑셀 매핑)·tabular-nums 유지 **E6** 글로벌 저장선 상단 sticky 헤더 고정(임시저장/DB 저장/네이버 엑셀·직접등록 only)·하단 저장바 제거 **E7** 저장 후 온실 아틀리에=이미지 탭 전용 primary CTA. 바인딩 보존(판매가→실마진·꿀통지수, 검색최적화→SEO 점수 라이브). #135 edit 자동전파. 순감 -131줄. Desktop 검증대기 |
| P1-d CRAWL-GRID refine | /crawl 이력 그리드 정렬 + #62 OverflowMenu 포털 | P1 | P1-c·baa8a3c(독립) | ✅ DONE·**Desktop PASS**(tsc0·build0·prod 1e65c20) — 권위 §P1-d. 로직 무변경(#132). **D1** 헤더 2행 정렬(Row1 타이틀+필터뱃지 / Row2 공급사+상품명검색(grows)+초기화·edge정렬) **D2** 밀도토글(조밀/여유) 제거→단일 density(rowPadY 9)·histDensity/Rows3·Rows4 제거 **D3** 정렬헤더(도매가/예상마진율) justifySelf:end 우측정렬→우측 tabular 데이터 정합·예상마진율 셀 우측정렬 **D5** 일괄등록바(등록대기로/한번에 임시등록/등록 시작)=fixed 하단 중앙 플로팅(선택시만·해제시 사라짐, 기존 sticky-top 교체) **D4/#62** 공용 OverflowMenu 드롭다운=createPortal(document.body)·position:fixed·trigger rect 측정·scroll/resize 동기화→그리드 overflow:hidden 클리핑+형제행 z-index 가림 해소(전역: products/new 상단바·studio AssetBrowser 회귀안전). Desktop 검증대기 |
| P1-a refine | USection 통일형(③)·기본정보 재배치·#137 케밥 | P1 | P1-a·P1-e | ✅ DONE(2단계 #138·tsc0·clean build0)·**검증요청** — 권위 §P1-a LOCKED(brief rev4). ③통일형 확정. **P1-a.1**(a9f917a): USection 정본카드 신설[아이콘+타이틀 헤더·번호뱃지 제거·강제 아코디언 제거(본문 상시)·--border-neutral #E4DFD4 1px·흰 카드·tabular-nums·브랜드핑크=액티브/CTA/필수* 한정] + RSection/DSection→USection 위임(레거시 별칭·전 ~15 섹션 4탭 단일 셸·아이콘 중립화). **P1-a.2**(fc8d206): [2 기본정보] 정체성 우선 재배치=가격→플랫폼·공급사→상품코드→브랜드·원산지·수입사→대체상품→옵션(OptionManager LAST)·번들 카드 분할·중복 basic fragment 2→1 + #137=플랫폼/공급사 선택항목 수정·삭제 → 공용 포털 OverflowMenu(D4) 케밥(새 메뉴 0·핸들러 verbatim) + 탭3·4는 컴포넌트 변경으로 USection 자동상속. 스킨/구조/재배치만(#132)·필드/핸들러/검증/바인딩 무접촉·#135 edit 자동전파. preview 실DOM: 탭2 순서 실측(가격·플랫폼·공급사·상품코드·브랜드·대체상품·옵션)·번호뱃지 0·콘솔 기존 SVG warn만. **Desktop PASS**(2026-06-24). ⚠️ #137 갭: 케밥 적용은 플랫폼/공급사 선택항목(수정·삭제)만 — 브리핑이 예시한 대체상품/옵션/asset 행 보조액션은 자식 컴포넌트(AlternativeProductPanel·OptionManager·AssetBrowser) 소관이라 미적용, 후속 레인에서 동일 포털 OverflowMenu로 정리 |
| CR-1 마진추정 정밀화 | 예상마진율 ~13% 군집·40%강조 미발화 | P2(백로그) | P1-c | 📋 BACKLOG·운영자 승인대기 — calcPrefillSalePrice 보수적 추정→예상마진율 대부분 ~13% 군집·40%+ 강조 미발화·정렬 변별력 약. 제안: 시장가/현실적 마크업 기반 추정으로 변경. P1-d D1-D5와 분리·운영자 승인 후 착수 |
| #34 정리 | stray 백업/중복 파일 삭제(#43 정합) | P2 | - | ✅ DONE — `git rm` 5건: `products/Product`(무확장·오발사 ls), `products/ProductCard 2.tsx`(공백·dup), `products/new/page.tsx.bak2`, **+발견** `product/ProductForm.tsx.bak2`·`product/ProductForm.tsx.broken`(동일 #43 백업패턴·참조0). 전수 스캔 `\.(bak|bak2|old|tmp|broken)$`+`* N.tsx` = CLEAN. 실상품 .tsx·`product/` 정상 컴포넌트 무접촉 |
| S2-B | 중앙 클렌징(#142): 무거운 폼(Firefly/무드/진단/상세/배경/9슬롯)→좌 배양실 step-sync · 중앙=미리보기+조립슬롯+꼬띠버블 · 폼필드까지 #144 | P1 | S2-A.1 | 🟡 S2-B.1 **Desktop PASS**(559a284·노트2 해소)·S2-B.2 DONE(7a95368·검증요청)·S2-C 다음. **S2-B.1**(relocation skeleton): 단계 카드 center→배양실 reparent·중앙=꼬띠+미리보기+조립슬롯 정적 스텁·핸들러/상태 무접촉(#132)·tsc0/build0/preview 실DOM 검증. **S2-B.2**(후속·비차단): 배양실 step-sync reveal(각 step 관련폼만 노출)·신호등 StatusCards·꼬띠 마이크로카피·표시폰트 결정(Cafe24 Ssurround vs Gmarket Sans·운영자 보류) → S2-C → S2-D(조립슬롯 충전·center hover Firefly 기존액션만 #132). PARALLEL-READY(운영자 GO·P1-a refine과 동시 가능·독립 /studio·운영자 결정 불요·#124 무충돌·#132) — 다음 Code 레인. 브리핑 rev4 §PARALLEL: 무거운 폼 center→좌 배양실 step-sync, center=미리보기+조립슬롯+꼬띠버블, #144 필드단위 → 이후 S2-C(step-sync 폼+신호등 StatusCard+브랜드 디스플레이폰트+꼬띠 마이크로카피) → S2-D(center hover Firefly 누끼/합성 기존액션만 #132+조립슬롯). **S2-B.2 완료**(7a95368): 배양실 상단 신호등 StepStatus 스트립(미작업/진행/완료·공용 StatusBadge·기존상태만·#55)+REVEAL(클릭=step 폼만 노출·페이드)+꼬띠 센터버블·i18n stepStatus. 노트 해소: (a) w:0 깔끔형=모바일 lg:hidden 사본(의도·#140) (b) 센터 꼬띠 버블 렌더 확인 |
