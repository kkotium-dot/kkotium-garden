...
**Status**: OPEN (2026-06-02 등록, P0 엑셀 매핑 누락 5건 수술 turn에서 발견)
**Severity**: medium (현재 5건은 수술됨, 그러나 신규 필드 추가 시 동일 패턴 재발 가능)
**Owner**: 매 PR 리뷰 — 신규 NaverProductData 필드 추가 시 route.ts 동시 갱신 의무
...
2026-06-02 발견 5건 (수술 완료):
- F1 optionType/optionNames/optionValues/optionPrices/optionStocks
- F2 manufacturer (잘못된 컬럼)
- F3 productName (우선순위 역전)
- F4 returnFee/exchangeFee (text fallback 없음)
- F5 noticeBrandName/noticeModelName/noticeManufacturer
...
(c) **빈 칸 단정 의무**: production POST /api/naver/excel 후 openpyxl 또는 ExcelJS readback으로 핵심 카테고리(옵션/제조사/배송/정보고시) 충진 단정.
...
**SOURCE**: 2026-06-02 Code turn (P0 엑셀 매핑 5건 수술), 권위 `docs/handoff/HANDOFF_naver_excel_mapping_gaps_2026-06-02.md`
