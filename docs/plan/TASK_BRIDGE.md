...
| E1·E2 | 대시보드 처분 카운트 정합 · 위젯 전수 점검 | `DONE` | A1 | 🌸 | `d1067ab`·`8fa392c` · #290/#292 |
| **E3** | 죽은 컴포넌트 13개 처리 방침 | `WAIT-OP` | **운영자 판단** | 🌸 | 삭제 / 보존 / 되살림 중 택. 현재 `@unmounted` 표식만 부착 |
| **G1-B** | 꽃단장 UI 구조 개편 | `HOLD` | **운영자 정리 중** | 🌸 | 스펙 나올 때까지 착수 금지 |
| D3 | 폴링 배치/상한 | `CONDITIONAL` | 상품 수백 개 도달 | 🌸 | 지금은 과잉 |
| E4·E5 | 클로드디자인 v7 / z3c stash | `HOLD` | — | — | 보류 |

> **현재 READY 항목 0건.** 운영자 판단 대기 2건(E3·G1-B).

> **현재 양쪽 작업 큐가 모두 비었습니다.** `G1-B` 스펙이 정의되면 재개합니다.

**다음 세션 권장 순서**: 운영자 판단 대기(`E3` 죽은 코드 처리 / `G1-B` 꽃단장 스펙). READY 항목 없음.

**병행 규율(#284 보강 · #289 · #291)**:
- 소유권은 **동시 편집 방지**이지 영구 배타가 아니다 → (1)큐 확인 (2)미커밋 확인 (3)인계 명시
- 순번은 충돌한다 → **커밋 SHA가 정본**(#289)
- **인계는 작성 시점 스냅샷**(#291) → `BASELINE: main <SHA>` 명시, 받는 쪽은 pull + 보드 `READY` 확인
- **검증 전 "실제로 마운트되는가" 먼저 확인**(#292) — 죽은 코드 검증에 시간을 쓰지 않는다
