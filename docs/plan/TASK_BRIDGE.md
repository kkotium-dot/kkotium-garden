...
| E1 | 대시보드 처분 카운트 정합(#290) | `DONE` | A1 | 🌸 | `d1067ab` · #278 서버쪽 재발 수정 |
| **E2** | 나머지 대시보드 위젯 정합성 | `READY` | E1 | 🌸 | DailyPlan·KkottiWidget·ZombieReactivationCard 등이 처분/발행 게이트와 어긋나는지 |
| **G1-B** | 꽃단장 UI 구조 개편 | `HOLD` | **운영자 정리 중** | 🌸 | ⚠️ 개선 범위가 커서 운영자가 직접 정리. 스펙 나올 때까지 착수 금지 |
| D3 | 폴링 배치/상한 | `CONDITIONAL` | 상품 수백 개 도달 | 🌸 | 지금은 과잉 |
| E3·E4 | 클로드디자인 v7 / z3c stash | `HOLD` | — | — | 보류 |

> **현재 양쪽 작업 큐가 모두 비었습니다.** `G1-B` 스펙이 정의되면 재개합니다.

**다음 세션 권장 순서**: `E2`(나머지 대시보드 위젯 정합성) — E1과 같은 결함이 다른 위젯에 남아 있을 가능성.

**병행 규율(#284 보강 · #289 · #291)**:
- 파일 영역 소유권은 **동시 편집 방지**이지 영구 배타가 아니다. 상대 영역 필요 시
  (1) 보드에서 상대 큐 확인 (2) `git status` 미커밋 확인 (3) **인계에 명시**
- 순번은 충돌한다 → **커밋 SHA가 정본**(#289)
- **인계는 작성 시점 스냅샷이다**(#291) → 인계에 `BASELINE: main <SHA>` 명시,
  받는 쪽은 착수 전 `git pull` + 보드에서 `READY` 여부 확인
