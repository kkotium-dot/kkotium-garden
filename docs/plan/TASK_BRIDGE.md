...
|---|---|---|---|---|---|
| A~C4 | 처분엔진·배지레일·원클릭·대기함·R검증·부활소 | `DONE` | — | 🌸 | #273~#285 |
| C5·C5-b·C5-c | 발행게이트 · 개입점 점검 · 조용한 실패 색출 | `DONE` | — | 🌸 | #286/#287 |
| D1·G1-A·F2·F3 | 페르소나 · 꽃단장 문구 · 독자축 · 라벨통일 | `DONE` | — | 💻 | #262/#283/#288 |
| D2·E3 | 도매꾹 폴링 유지 · 죽은코드 B(현상유지) | `DONE` | 운영자 승인 | 🌸 | 결정 완료 |
| E1·E2·E4·F4 | 대시보드·위젯·디스코드·앱알림 판정 정합 | `DONE` | A1 | 🌸 | #290/#292/#293/#294 · **6채널 전수 완료** |
| **DESKTOP-1** | 처분 판정 **실데이터** 7채널 검증 | `READY` | E1~F4 | 🖥 **Desktop** | ⚠️ Cowork는 Supabase 쓰기 차단 → **Desktop만 가능**. 인계: `docs/handoff/DESKTOP_HANDOFF_2026-07-22.md` |
| **G1-B** | 꽃단장 UI 구조 개편 | `HOLD` | **운영자 정리 중** | 🌸 | 스펙 나올 때까지 착수 금지 |
| D3 | 폴링 배치/상한 | `CONDITIONAL` | 상품 수백 개 도달 | 🌸 | 지금은 과잉 |
| E5·E6 | 클로드디자인 v7 / z3c stash | `HOLD` | — | — | 보류 |

> **🌸 Cowork·💻 Code 큐 소진.** 유일한 READY는 `DESKTOP-1`(Desktop 전용).

> **현재 READY 0건.** `G1-B`(운영자 정리 중)만 남았고, 처분 판정 정합성은
> **화면·서버 집계·알림 3채널 전부 완료**(#278→#290→#293).

> **현재 READY 항목 0건.** 운영자 판단 대기 2건(E3·G1-B).

> **현재 양쪽 작업 큐가 모두 비었습니다.** `G1-B` 스펙이 정의되면 재개합니다.

**다음 세션 권장 순서**: 🖥 **Desktop이 `DESKTOP-1`** 착수(유일한 READY). 🌸/💻는 큐 소진 — `G1-B` 스펙 대기.

**3환경 병행 규율**:
- 소유권은 **동시 편집 방지**(#284 보강) → (1)보드 큐 확인 (2)`git status` 미커밋 확인 (3)인계 명시
- 순번 충돌 → **커밋 SHA가 정본**(#289)
- 인계는 **작성 시점 스냅샷**(#291) → `BASELINE: main <SHA>` 명시, 받는 쪽은 pull + 보드 `READY` 확인
- 검증 전 **마운트 여부 확인**(#292) · 점검 목록은 **그 턴에 전수 실행**(#294)
- 🖥 Desktop은 **코드 수정 불가** → 결함 발견 시 고치지 말고 위치·재현절차를 인계에 기록

---

## §3 ACTIVE HAND-OFF ⭐ (항상 최상단 한 섹션, 매 hand-off 시 갱신)

### 2026-07-22 (130) 🌸 Cowork — 체크리스트 전수 실행(F4·#294) + 🖥 Desktop 인계 (FROM 🌸 Cowork, main `88a588d`, tsc0·build0·prod 배포)
- **★F4 DONE(#294)**: `#293`에서 만든 점검 목록을 **실제로 전수 실행**(`grep "status === 'OUT_OF_STOCK'"` 20곳 분류)하니 **사각지대 2곳 추가 색출**. `/api/notifications`(앱 헤더 **종 알림**)와 `/api/weekly-report`(**주간 리포트**)가 status 기준이라 공급처 단절 상품이 누락됐다. 문구도 "품절 상품"→**"처분 필요 / 품절이거나 공급처가 끊긴 상품이에요"**로 교체(품절만으론 단절을 포괄 못 함).
- **★교훈**: 체크리스트를 **문서에 적는 것과 실제로 돌리는 것은 다른 일**. `#293`을 쓰면서 목록을 만들었지만 그 순간 훑지 않았고, 다음 턴에 돌리자마자 2건 나왔다. → **점검 목록은 만든 턴에 최소 1회 전수 실행**하고 **"정상인 곳"도 분류 기록**(다음 사람이 같은 곳 또 안 뒤지게).
- **처분 판정 정합성 6채널 전수 완료**: `#278` 대기함 → `#290` 대시보드 → `#292` 위젯 → `#293` 디스코드 → `#294` 앱알림·주간리포트.
- **검증**: tsc0·build0 / `/api/notifications` 실호출 200(처분 대상 0건이라 알림 없음 — 정확) / weekly-report는 **인증 필요한 크론 엔드포인트라 직접 호출 불가** → 합집합 로직 합성 검증(전 항목 PASS, 판정 0건 시 status degrade PASS) / 화면 회귀 0.
- **★검증 공백 → Desktop 인계**: Cowork는 **Supabase 쓰기 차단**이라 스텁·합성으로만 검증. **"공급처 단절 상품이 실제로 존재할 때 7채널이 모두 같은 수를 말하는가"는 아직 실측 못 했다.** → `docs/handoff/DESKTOP_HANDOFF_2026-07-22.md` 작성(주입 SQL·7채널 확인표·원복 절차·금지사항).
- **다음 1액션**: 🖥 Desktop이 `DESKTOP-1` 착수. 🌸/💻 큐 소진.
- **의존성**: (129) 위에 이어짐.

### 2026-07-22 (129) 🌸 Cowork — 디스코드 알림 정합(E4·#293) + E3 결정 종결 (FROM 🌸 Cowork, main `4fc37c6`, tsc0·build0·prod 배포)
- **★E4 DONE(#293) — #290의 세 번째 인스턴스, 가장 위험**: 매일 가는 디스코드 재고 알림이 `status === 'OUT_OF_STOCK'` 기준이라 **공급처 단절 상품에 알림이 아예 울리지 않았다.** 알림은 운영자가 앱을 안 열어도 받는 유일한 채널이라, 침묵하면 가장 급한 건을 통째로 놓친다. → `loadDispositionVerdicts()` 재사용해 **알림 대상 = status OOS ∪ 처분 판정 대기**로 확장, `daysOos`에 실제 품절 지속일(#273) 주입.
- **★이벤트 기록은 status 기준 유지**: `productEvent(type=OOS)`는 "status가 바뀜"의 사실 기록. **알림 대상과 이벤트 대상은 다른 축**이라 분리.
- **#293 신설**: 앱은 모든 채널에서 같은 말을 해야 한다. **알림이 가장 늦게 정합된다** — 화면은 열면 보이지만 알림 결함은 "안 울리는" 형태라 눈에 안 띈다. 판정 기준 변경 시 **화면·서버집계·알림 3곳을 같은 턴에** 확인할 것.
- **E3 DONE — 운영자 승인 B(현 상태 유지)**: 죽은 컴포넌트 13개는 삭제하지 않고 `@unmounted` 표식 유지.
- **검증**: tsc0·build0 / 판정 로더 실데이터(3건·처분대기0) / **합집합 필터 로직 검증**(합성, 발송 없음): 알림 `a,b,d` PASS·이벤트 `a,d` PASS·**공급처 단절 포함 PASS**·중복0 / 화면 회귀 0. ※ **디스코드 실발송 안 함**(운영자 실알림 방지).
- **다음 1액션**: READY 없음. `G1-B` 스펙 대기.
- **의존성**: (128) 위에 이어짐.

### 2026-07-22 (128) 🌸 Cowork — 위젯 전수 점검(E2) + 죽은 코드 13개 표식(#292) (FROM 🌸 Cowork, main `8fa392c`, tsc0·build0·prod 배포)
- **E2 DONE**: `DailyPlanWidget` slot C(재활성화)·`ZombieReactivationCard`는 부활소 링크 **정상**. **`KkottiWidget` "품절 현황"만 E1과 동일 결함**(status 카운트 + 목적지 오류) → 판정 기준으로 교체, 대기함 링크, 라벨 "처분 대기".
- **★정직 보고**: KkottiWidget은 **어디에도 마운트되지 않은 죽은 코드**였다(#228로 제거). 수정 후 브라우저 검증하려다 발견. **이 수정은 현재 화면에 영향 없음** — 수정 자체는 옳아 유지(되살릴 때 안전). **검증 순서가 틀렸다**: 마운트 여부를 먼저 확인했어야 함.
- **#292 신설**: 죽은 코드는 **결함의 냉동고**다. 미마운트 컴포넌트는 결함이 보존되고 되살릴 때 **이미 고친 결함이 함께 부활**한다. 전수 스캔 → 대시보드 **13개** 발견. 삭제 대신 `@unmounted` 표식 + "되살리기 전 확인할 것" 체크리스트 부착(삭제는 운영자 판단 → `E3`).
- **검증**: tsc0·build0 · 대시보드 회귀 0(KPI "처분 대기"·대기함 링크·오늘 할 일 정상) · **콘솔 에러 0**.
- **다음 1액션**: READY 없음. 운영자 판단 대기(`E3` 죽은 코드 처리 / `G1-B` 꽃단장 스펙).
- **의존성**: (127) 위에 이어짐.

### 2026-07-22 (127) 🌸 Cowork — 대시보드 처분 카운트 정합(E1·#290) (FROM 🌸 Cowork, main `d1067ab`, tsc0·build0·prod 배포)
- **★E1 DONE(#290) — #278의 서버 쪽 재발**: 대기함 화면은 판정으로 고쳤는데 **대시보드는 여전히 `status='OUT_OF_STOCK'`로 셌다**. 공급처 단절 상품(status=ACTIVE)이 **앱 첫 화면 카운트에서 통째로 누락** — 앱이 자기 할 일을 실제보다 적게 보고했다.
- **원인**: 판정 로직은 PURE로 분리했으나 **입력(재고 스냅샷) 수집 경로를 화면에만 만들어** 서버가 옛 방식에 머물렀다. → `disposition-load.ts` 신설(서버 판정 단일 권위, 상품1+스냅샷1 쿼리, 화면과 **동일 계산 함수 재사용**).
- **배선**: stats API `dispositionPending` 추가 · **KpiStrip + TodayQueue 둘 다** 교정(카운트 소스·라벨·목적지). 같은 결함이 두 컴포넌트에 있어 함께 수정. ※ 좀비/부활소 카드의 `/products/reactivation` 링크는 정상이라 미변경.
- **#291 신설**: Cowork 인계가 Code 완료 보고보다 먼저 작성돼 **한 턴 낭비**. → 인계에 `BASELINE: main <SHA>` 명시, 받는 쪽 착수 전 pull+보드 확인, 보내는 쪽 인계 전 보드 확인.
- **운영자 지시 반영**: `G1-B` → **HOLD**(개선 범위가 커서 운영자가 직접 정리 중).
- **검증**: 실데이터 "처분 대기 0건"·링크 대기함 / 스텁 `dispositionPending=2` → **"처분 대기 2건" 정확 반영** / 원복·구라벨 소멸·콘솔0 / tsc0·build0.
- **소유권 메모**: `KpiStrip`/`TodayQueue`의 `*.strings.ko.json`(Code 영역) 수정 — Code 큐 비어 있어 위험 0, #284 보강 절차 준수.
- **다음 1액션**: `E2`(나머지 대시보드 위젯 정합성).
- **의존성**: (126) 위에 이어짐.

### 2026-07-22 (126) 🌸 Cowork — 조용한 실패 전수 색출(C5-c) + 라벨 통일(F3) (FROM 🌸 Cowork, main `cb2e536`, tsc0·build0·prod 배포)
- **C5-c DONE — 두 번째 인스턴스 발견·수정**: `Promise.all` 34곳 전수 grep. 대부분은 `const [a,b] = await Promise.all([...])` 병렬 읽기 + `if (x.success)` 가드로 안전. **`ApplyTemplateModal`(배송 템플릿 일괄 적용)만 결과 미사용 + mutation** → 전부 실패해도 "적용 완료"로 닫혔다. `allSettled` + `res.ok` 확인 + 부분 실패 보고로 수정.
- **★근본 원인 명확화**: `fetch`는 HTTP 500에도 reject하지 않는다. 그래서 `Promise.all`이 정상 resolve되고 **실패가 성공으로 둔갑**한다 — #287의 진짜 범인.
- **F3 DONE(#288)**: 라벨 표기 통일(`판매 중`→`판매중`, `판매 중지`→`판매중지`). 나머지 107곳은 이미 붙여쓰기. **문장 속 서술("아직 판매 중이에요")은 띄어쓰기가 맞으므로 미변경** — 라벨과 문장은 다른 축.
- **#289 신설**: 병행 세션 순번 충돌 실증 — `(124)` 엔트리가 두 개 생김(Code·Cowork 동시 부여). **식별자는 SHA가 정본**, 순번은 보조. 시간순 뒤쪽이 +1(이번엔 Cowork를 125로 조정).
- **#284 보강**: 파일 영역 소유권은 "동시 편집 방지"이지 영구 배타가 아니다. 상대 영역을 건드릴 때는 (1)큐 확인 (2)미커밋 확인 (3)인계에 명시. ※ 이번에 Cowork가 `InventoryBadge.strings.ko.json`(Code 영역) 수정 — Code 큐 비어 있어 위험 0.
- **검증**: C5-c(1건 강제실패→"2개 중 1개 적용 완료, 1개 실패") / F3(드롭다운 5옵션 통일 확인) / 스텁 원복·콘솔0 / tsc0·build0 / 라벨 재검사 0건.
- **다음 1액션**: 🌸 Cowork 큐 비었음. `G1-B` 스펙 대기 또는 신규 지시.
- **의존성**: (125) 위에 이어짐.

### 2026-07-22 (125) 🌸 Cowork — 개입점 전수 점검 완료 + 일괄변경 신뢰성(#287) (FROM 🌸 Cowork, main `4a1955a`, tsc0·build0·prod 배포)
- **C5-b DONE — 전수 점검 3곳 중 1곳 결함**: `/crawl` 담기(재고 위험/주의 배지 ✅)·`/products/sourced` 일괄변경(실패 보고 ✅)은 통과. **`/products` 일괄 상태변경만 무방비**(#62 반대 사례 — 같은 동작이 두 곳에 다르게 구현됨).
- **#287 신뢰성 결함 2건 수정**: (1) `Promise.all`로 쏘고 **결과를 안 봐서** 실패해도 알림 없이 화면만 바뀌어 **DB와 어긋남** → `allSettled` + 실패분 롤백 + "N개 중 M개 완료, K개 실패" 보고. (2) 판매중 전환에 **공급 게이트 부재** → `checkPublishGate` + 상품명 명시 확인창, 취소 시 PATCH 0건.
- **검증**: 게이트(확인창 상품명 2건·취소 시 PATCH 0) / 부분실패(1건 강제실패→정확한 보고+실패분만 롤백) / 스텁 원복·콘솔0 / tsc0·build0.
- **자기 교정 기록**: "판매중" 옵션이 없는 줄 알았으나 실제 라벨이 `판매 중`(공백)이라 **제 정규식이 놓친 것**. 코드가 아니라 테스트가 틀렸다(#282 연장). 부수로 라벨 표기 혼재 발견 → `F3` 등재.
- **패치 위치**: `src/app/products/page.tsx`(handleBulkStatusChange).
- **다음 1액션**: `C5-c`(Promise.all 결과 미사용 전수 grep — #287 확장) 또는 `F3`.
- **의존성**: (123) 위에 이어짐.
...
- **D2 DONE — 폴링 유지(ON)·DRAFT 포함, 코드 변경 없음**: 크론 5일 연속 정상(재고 실변동 반영) 실측. **끄면 발행게이트·대기함·부활소가 전부 무력화**되므로 유지가 정답. 규모 대비 배치/상한은 `D3` 조건부 등재.
- **G1 분해 — 예상 못 한 #262 위반**: 꽃단장 3파일에 **개발 은어 9건**이 셀러 화면 노출 중("Supabase Storage"·"cooldown"·"DB row"). → `G1-A`(문구 정비, READY, 💻) / `G1-B`(UI 구조 개편, WAIT-OP, **스펙 미정의**)로 분리. 운영자의 "나중에 한 번에"는 구조 변경을 뜻하므로 G1-A는 지금 해도 이중작업 아님.
- **환경 수리**: `@types` macOS 중복 폴더 20개(빈 디렉터리) 삭제로 tsc 복구. #279 패턴 재확인.
- **검증**: tsc0·build0 · 브라우저 3분기(정상/차단/override) · 스텁 원복 · 앱 콘솔 0.
- **다음 1액션**: `C5-b`(잔여 개입점) ↔ `G1-A`(💻) 병행.
- **의존성**: (122) 위에 이어짐.

### 2026-07-22 (122) 🌸 Cowork — 부활소 처분 권고 노출(C4·#285) (FROM 🌸 Cowork, main `0648c49`, tsc0·build0·prod 배포)
- **C4 DONE(#285)**: 좀비 부활소가 되살릴 상품을 고르는 화면인데 **공급처 단절·처분 권고가 없었다**. 공급처 끊긴 상품을 되살리려 수십 분 쓰는 일이 구조적으로 안 막혔음. Product 타입에 판정 입력 추가 + `useInventoryBadges` 연결 + **기존 InventoryBadge 재사용**(판정은 disposition.ts 단일 권위, 화면 자체 규칙 0). NameDiagnosisBadge에 compact 적용(#276).
- **원칙 #285**: 행동을 유도하는 화면에는 **그 행동을 말려야 할 신호도** 함께. 긍정 신호만 있는 화면은 운영자를 손실로 안내한다. 개입점을 늘릴 때 **로직은 늘리지 않는다**(화면마다 판정이 갈라지면 같은 상품에 다른 답).
- **검증**: tsc0·build0 · 실데이터 재고 배지 정상 · fetch 스텁으로 "공급처 단절 · 삭제 권장" 노출 확인 · 스텁 원복 후 실데이터 복귀 · 콘솔 0.
- **패치 위치**: `src/app/products/reactivation/page.tsx`.
- **다음 1액션**: `C5`(개입점 역신호 전수 점검 — #285를 전 화면으로 확장).
- **의존성**: (121) 위에 이어짐.

### 2026-07-22 (121) 🌸↔💻 페르소나 독자 경계 확립(#283) + D1 종결 + 병행 규율 실증(#284)
- **★사고 직전 차단**: 💻 Code가 D1 인계 문서의 파일 설명 오류를 **실제 문구 추출로 발견**. `section-renderers`·`preset/samples`·`detail-content-templates`는 "앱 문구"가 아니라 **네이버 고객이 보는 상품 상세페이지 카피**(배송/교환·반품 안내 포함). 강행했으면 교환·반품 고지에 "까꿍💖"이 들어갔다. Code가 강행 대신 **보고 후 판단 요청** → Cowork가 파일 직접 검증 → **보류 확정**.
- **#283 신설**: 페르소나는 셀러 대면 앱 UI에만, 고객 대면 상품 카피는 금지. #262의 "화면 문구"는 셀러가 보는 앱 UI를 뜻함.
- **근본 수정(F2)**: 원인은 `persona-audit.py`에 독자 축이 없던 것. `CUSTOMER_FACING` 제외 + **별도 섹션 노출**(침묵 제외 금지). 셀러 대면 0건 **6→3개**.
- **D1 DONE**: Code 1~9 적용(build0/tsc0). 잔여 3개는 전부 꽃단장 → G1 종속. 10·14·15 영구 제외.
- **#284 병행 규율 실증**: Code가 `products/page.tsx` 미커밋 132줄(Cowork C2 진행 중)을 발견하고 **손대지 않고 자기 파일만 개별 add** → 충돌 0. `git add -A` 금지, 남의 미커밋은 stash 금지·그대로 두고 보고.
- **인계 문서 rev2 정정**: `docs/handoff/CODE_HANDOFF_D1_PERSONA_2026-07-22.md` 전면 재작성.
- **다음 1액션**: `C4`(재활성화 목록 처분 권고 노출).
- **의존성**: (120) 위에 이어짐.

### 2026-07-22 (120) 🌸 Cowork — 배지 레일 모바일 확장(C2) + R-1/R-2/R-3 종결(C1·#281) (FROM 🌸 Cowork, main `b2a3a80`, tsc0·build0·prod 배포)
- **C2 DONE(#280)**: 모바일 카드에 배지가 **아예 없던** 정보 비대칭 해소. `buildProductBadgeItems()`로 데스크톱·모바일이 같은 배지 목록을 쓰게 통합(#62). 모바일 폭별 자동 접힘 실측(420→5 / 340→3+"+2" / 260→2+"+3" / 200→1+"+4", 잘림 0), 우선순위 유지 확인.
- **C1 DONE**: **R-1 통과**(시장분석 자사 제외, sampleSize=9, 대조군 정상) · **R-2 통과**(하단 컨트롤 3개) · **R-3 실패 → #281로 근본 수정**.
- **#281 DONE**: 패널 720px 열리면 목록 495px만 남는데 9컬럼은 1020px 필요 — 밀어내기로는 물리적 불가. 패널 열림 시 **컬럼 자체 축소**(5트랙). 숨긴 정보는 패널에 전부 있어 손실 0. 트랙·헤더라벨·셀개수 3자 동시 분기(하나만 바꾸면 정렬 깨짐·타입검사 미검출).
- **#282 검증 교훈**: 연속 폭 변경 + 짧은 대기 측정이 **거짓 실패**를 만들었다. ResizeObserver→setState→커밋 사슬이 비동기. **"버그다" 결론 전 단일 단계 재현 필수.**
- **환경 재발 기록**: dev 실행 중 `npm run build` → `.next` 충돌로 dev 재파손. **build 전 dev kill 필수**(#279와 함께 dev 파손 2대 원인).
- **패치 위치**: `src/app/products/page.tsx`(빌더 공통화·모바일 레일·컴팩트 모드).
- **다음 1액션**: `C4`(재활성화 목록 처분 권고 노출). `D1`은 💻 Code 병행 중.
- **의존성**: (119) 위에 이어짐.

### 2026-07-22 (119) 🌸 Cowork — 처분 결정 대기함(B3) + dev 환경 근본 수정(C3) + 원칙 문서 분할(F1) (FROM 🌸 Cowork, main `9c3e1dd`, tsc0·build0·prod 배포)
- **Target**: §3-A 보드 `B3` + 환경 복구 + #31 임계 해소.
- **B3 DONE(#278) ★누락결함 수정**: 기존 품절 페이지가 `status=OUT_OF_STOCK`만 조회 → **공급처 단절 상품(앱 status=ACTIVE)이 통째로 누락**되고 있었음. 전 상품+처분 판정으로 수집 축 전환. 5그룹 분류 + 그룹 전체선택 + 일괄 반영(전 건 dryRun→확인→순차실행, #46 이중게이트). no-op은 GO 비활성. 권고 없는 품절은 "재입고 검토"로 보존(기능 삭제 0).
- **C3 DONE(#279) 근본원인 규명**: 셸 `NODE_ENV=production`이 원인. Next dev가 CSS 파이프라인을 끄고 `npm ci`가 devDeps를 건너뜀. `.env`는 셸에 밀림. → `package.json` dev 스크립트에 `NODE_ENV=development` 못박아 **전 환경 재발 차단**.
- **F1 DONE(#31)**: PRINCIPLES_LEARNED 1713→913줄, 815줄을 archive로. **본문 바이트 무손실 검증 PASS**. 양 파일에 번호 인덱스·grep 방법 명시.
- **검증**: tsc0·build0 · dev 200/CSS오류0 복구 · 브라우저 실측(5그룹 분류·통계·선택·일괄 미리보기·no-op GO비활성) · 스텁 원복 후 실데이터·콘솔0 · **prod DB 무오염**.
- **D1 11~13 보류 확정(운영자 재문의 불필요)**: 꽃단장 27문구는 `G1`(꽃단장 UI 개편)에 종속. 지금 문구만 바꾸면 개편 때 두 번 손댐. **D1 범위 = 1~10 + 14~15**.
- **패치 위치**: `src/app/products/out-of-stock/page.tsx`(전면 재작성) · `package.json`(dev script) · `docs/plan/PRINCIPLES_LEARNED.md`+`archive/`.
- **다음 1액션**: `C2`(배지 레일 확장) 또는 `C1`(육안). `D1`은 💻 Code 병행 중.
- **의존성**: (118) 위에 이어짐.
...
- **I-1 필터 축 분리(#233) DONE**: 필터 칩 목록에서 담기방식(단건/대량) 2개를 제거하고 진행단계(SOURCED/PENDING/REGISTERED/HOLD) 4개+전체만 남김. `histFilter` 타입에서 `'single'|'bulk'` 제거, `loadShelf()`의 source 쿼리 분기도 함께 제거(죽은 코드 정리). 단건/대량은 `상품정보` 컬럼의 목록 행 배지("한 송이"/"한아름")로 하향 — `log.source` 값 기준, 기존 옵션/묶음/무료공급 pill과 같은 자리에 추가. **기본값 확인**: 진단(rev60)이 "지금 대량이 선택돼 빈 화면"이라 지적했으나 실측 결과 `histFilter` 초기값은 이미 `'all'`이었음(수정 불요) — 최상위 tab도 기본 `'history'`(꿀통 꽃수레)라 착지 시 빈 화면 재현 안 됨. 해당 부분은 진단 시점 이후 이미 해소돼 있었던 것으로 판단, 별도 조치 없음.
- **I-2 라벨 세계관 통일(#262/#267) DONE**: 필터 칩 + 행별 "더보기 > 재분류" 드롭다운(RECLASSIFY_OPTIONS) 양쪽에 동일 라벨 적용 — 소싱완료→"꽃 골랐어요" / 등록대기→"정원 창고로 보낼 준비" / 등록완료→"정원 창고에 있어요" / 보류→"다음에 볼 꽃". 내부 enum 값(SOURCED/PENDING/REGISTERED/HOLD)은 무변경(DB 컬럼 free-text, 코드 로직 영향 없음), 표시 라벨만 교체. **스코프 밖으로 남긴 것**: `StageBadge`(공유 컴포넌트, Product.status 기반 다른 축 — 발행대기/발행됨/재활성화 등)는 이번 라벨셋과 무관해 손대지 않음(#55 — 다른 화면과 공유하는 컴포넌트를 지시 범위 밖에서 바꾸지 않음).
- **I-3 빈 상태 액션(#256) DONE**: 진짜 빈 상태(`logs.length===0`, 즉 서버가 필터 조건에 맞는 행을 0건 반환)에서만 `[꽃 한 송이 담기]`(→ `setTab('single')`) `[꽃 한아름 담기]`(→ `setTab('bulk')`) 두 버튼 렌더. 검색으로 인한 0건("검색 결과가 없습니다")에는 버튼 미노출(조건 분기 유지 — 검색은 "가져올 게 없다"가 아니라 "찾는 게 없다"라 다른 액션이 필요 없음).
- **검증**: tsc0 · `rm -rf .next && npm run build` 0 errors · 로컬 dev 브라우저 실측(Browser 도구) — 필터 칩 5개(전체/꽃골랐어요/정원창고로보낼준비/정원창고에있어요/다음에볼꽃)만 렌더·단건/대량 칩 0개 확인 / 행 배지 "한 송이" 정상 렌더 확인 / "다음에 볼 꽃"(count 0) 클릭 → 빈 상태 화면에 두 버튼 렌더 확인 / `[꽃 한아름 담기]` 클릭 → 실제로 "꽃 한아름 담기" 탭(URL 입력 폼)으로 전환 확인(스크린샷).
- **패치 위치**: `src/app/crawl/page.tsx`(필터 칩 배열·histFilter 타입·loadShelf·행 배지·RECLASSIFY_OPTIONS·빈 상태 블록).
- **다음 1액션**: [운영자] git push + `scripts/verify-vercel-deploy.sh --wait`(#36) 승인 → prod 반영 후 **육안 확인**(#265, 특히 라벨이 실제 화면 폭에서 줄바꿈 안 하고 잘 들어가는지 — "정원 창고로 보낼 준비"가 다른 라벨보다 길어 좁은 화면에서 wrap 가능성). 승인 시 다음은 [H-3 잔여](컬럼 재배치, 별도 검증 턴).
- **의존성**: (114) 위·rev60 진단 turn.
...
- **보강 반영(운영자 승인 조건 2건)**: (a) SidePanel 헤더를 "상태점만"→"썸네일+상품명+상태"로 재설계(#212 zone3 헤더 패턴 미러, 프리미엄 SaaS 구조화). (b) 좀비 사유 카드(product.tuningScore·#264)를 정보 탭 최상단(스크롤 없이 첫 화면)에 배치. ※ "기본 뷰가 발행 여부로 분리돼야 한다"는 보강 1(작성중 노출 제거)은 **2026-07-17 작업 F-2(ccf8e2c)에서 반영 완료** — TAB_CONFIG.all이 발행/연동 상품만 기본 노출하도록 교정됨("발행 전체"로 라벨 변경), ?tab=draft(꿀통창고 진입점)는 유지. (2026-07-23 정정)
...
- **다음 1액션**: ~~[운영자] 보강1 미착수분(발행 여부 기본 필터) 착수 여부 결정~~ → 2026-07-17 F-2(ccf8e2c)에서 반영 완료, 재결정 불요(2026-07-23 정정). [운영자] git push + `scripts/verify-vercel-deploy.sh --wait` 승인(#36) — 로컬 커밋만 된 상태, prod 미반영. [Desktop] prod 배포 후 4탭 브라우저 재검증(#88, 로컬 dev 검증은 본 턴 완료했으나 prod 별도 확인 필요).
...
| SD-04 | main 직접 push이 기본(브랜치 없음). feature 브랜치는 필요 시 예외적으로 사용(예: 화면 재설계·다중파일 묶음 작업), 이때도 최종 merge 승인은 항상 운영자 | 작업원칙 #4, #320 |
