...
| C5·C5-b·C5-c | 발행게이트 · 개입점 점검 · 조용한 실패 색출 | `DONE` | — | 🌸 | #286/#287 · `cb2e536` |
| D1·G1-A·F2 | 페르소나 · 꽃단장 문구 · 독자 축 | `DONE` | — | 💻/🌸 | `a692b7c` · 은어 9건 제거 |
| D2 | 도매꾹 폴링 판단 | `DONE` | — | 🌸 | 유지(ON)·DRAFT 포함 |
| F3 | 상태 라벨 표기 통일(#288) | `DONE` | — | 🌸 | `cb2e536` |
| **G1-B** | 꽃단장 UI 구조 개편 | `WAIT-OP` | **스펙 정의 필요** | 🌸 | ⚠️ **현재 유일한 대기 항목** — 아래 "운영자 확인 요청" 참조 |
| D3 | 폴링 배치/상한 | `CONDITIONAL` | 상품 수백 개 도달 | 🌸 | 지금은 과잉 |
| E1·E2 | 클로드디자인 v7 / z3c stash | `HOLD` | — | — | 보류 |

> **현재 양쪽 작업 큐가 모두 비었습니다.** `G1-B` 스펙이 정의되면 재개합니다.

**다음 세션 권장 순서**: 신규 지시 대기. `G1-B`는 스펙 정의 후 착수.

**병행 규율(#284 보강)**: 파일 영역 소유권은 **동시 편집 방지**이지 영구 배타가 아니다.
상대 영역을 건드려야 하면 (1) 보드에서 상대 큐 확인 (2) `git status`로 미커밋 확인
(3) **인계에 명시**. 순번은 충돌하므로 **커밋 SHA가 정본**(#289).
