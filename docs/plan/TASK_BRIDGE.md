...
| B2 | api-client SUSPENSION(#277) | `DONE` | — | 🌸 | `c36d380` · prod dryRun 검증완 |
| B1 | 처분 권고 원클릭 실행(#277) | `DONE` | B2 | 🌸 | `c36d380` · 실제 GO는 운영자 전용 |
| B3 | 처분 결정 대기함(#278) | `DONE` | A1·B1 | 🌸 | `9c3e1dd` · 누락결함 수정 + 일괄반영 |
| C3 | 로컬 dev CSS 복구(#279) | `DONE` | — | 🌸 | `9c3e1dd` · package.json에 NODE_ENV 못박음 |
| F1 | PRINCIPLES 문서 분할(#31) | `DONE` | — | 🌸 | 1713→913줄, 무손실 검증 PASS |
| **C2** | 배지 레일 다른 화면 확장 | `READY` | A2 | 🌸 | 모바일 카드·재활성화 목록. 데스크톱 목록만 적용됨 |
| **C1** | R-1/R-2/R-3 육안 확인 | `READY` | C3 | 🌸 | dev 복구돼 직접 촬영 가능. 최종 미감만 운영자 |
| **D1** | 페르소나 1~10 + 14~15 | `IN-PROGRESS` | — | 💻 Code | **11~13 제외 확정**(아래 G1에 종속) |
| **G1** | 꽃단장 UI 개편 + 페르소나 동시 적용 | `WAIT-OP` | 운영자 지시 | 🌸/💻 | D1의 11~13(27문구)이 여기 종속. 개편 때 한 번에 |
| D2 | 도매꾹 폴링 on/off | `WAIT-OP` | 운영자 지시 | 🖥 | 비용/DB 부하 |
| E1 | 클로드디자인 v7 PDF | `HOLD` | — | — | 보류 |
| E2 | z3c stash | `HOLD` | — | — | 보류 — `z3c-misdirected-changes-needs-redo` |

**다음 세션 권장 순서**: `C2`(배지 레일 확장) → `C1`(육안 확인). 둘 다 의존성 0.

**병행 가능 조합**(파일 겹침 0 — 동시 진행 안전):
- `C2`(components/*) ↔ `D1`(*.strings.ko.json, 💻 Code) — **완전 독립**
- `C1`(육안) — 언제든 단독
- `G1`(꽃단장) 착수 시 D1의 11~13을 흡수하므로, G1 지시가 나오면 D1 잔여와 합류시킬 것
