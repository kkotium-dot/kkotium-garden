...
| D2 | 도매꾹 폴링 판단 | `DONE` | — | 🌸 | 유지(ON)·DRAFT 포함 |
| E1·E2 | 대시보드 카운트 정합 · 위젯 전수 점검 | `DONE` | A1 | 🌸 | #290/#292 |
| **E3** | 죽은 컴포넌트 13개 처리 | `DONE` | 운영자 승인 | 🌸 | **B(현 상태 유지) 확정** — `@unmounted` 표식 유지, 삭제 안 함 |
| **E4** | 디스코드 알림 정합 | `DONE` | E1 | 🌸 | `4fc37c6` · #293 · 가장 위험한 축 |
...
**다음 세션 권장 순서**: `G1-B` 스펙 대기. READY 항목 없음.

**판정 기준 변경 시 점검 목록(#293)**: 화면 목록 · 대시보드 카운트 · KPI ·
**알림(디스코드/이메일)** · 크론 다이제스트 · 사이드바 배지 · 외부 API 응답
→ **같은 턴에 전부 확인**한다. 알림이 가장 늦게 발견되고 가장 위험하다.

**병행 규율(#284 보강 · #289 · #291 · #292)**:
- 소유권은 동시 편집 방지 → (1)큐 확인 (2)미커밋 확인 (3)인계 명시
- 순번 충돌 → **SHA가 정본**(#289) · 인계는 스냅샷 → `BASELINE: main <SHA>`(#291)
- **검증 전 마운트 여부 확인**(#292) — 죽은 코드 검증에 시간 쓰지 않는다
