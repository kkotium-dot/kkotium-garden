...
|---|---|---|---|---|---|
| A1·A2 | 처분 엔진(#273) · 배지 레일(#274~#276) | `DONE` | — | 🌸 | `4ee8585`·`7ebaae3` |
| B1·B2·B3 | 판매중지 push · 원클릭 실행 · 대기함(#277/#278) | `DONE` | — | 🌸 | `c36d380`·`9c3e1dd` |
| C1·C2·C3·R3 | R검증 · 모바일 확장 · dev수정 · 컴팩트(#279~#281) | `DONE` | — | 🌸 | `ab3702e`·`b2a3a80` |
| D1·F2 | 페르소나 · 독자 축 도입(#283) | `DONE` | — | 💻/🌸 | `3c4b4e6` · 잔여 3파일은 G1 종속 |
| F1 | PRINCIPLES 분할(#31) | `DONE` | — | 🌸 | 무손실 PASS |
| C4 | 부활소 처분 권고 노출(#285) | `DONE` | A1 | 🌸 | `0648c49` |
| **C5** | 개입점 역신호 전수 점검(#285 확장) | `READY` | C4 | 🌸 | "발행하기"·"소싱하기" 등 행동 유도 화면에 말리는 신호가 있는지 |
| **G1** | 꽃단장 UI 개편 + 페르소나 동시 적용 | `WAIT-OP` | 운영자 지시 | 🌸/💻 | 페르소나 잔여 3파일(37문구) 흡수 |
| D2 | 도매꾹 폴링 on/off | `WAIT-OP` | 운영자 지시 | 🖥 | 비용/DB 부하 |
| E1 | 클로드디자인 v7 PDF | `HOLD` | — | — | 보류 |
| E2 | z3c stash | `HOLD` | — | — | 보류 |

**다음 세션 권장 순서**: `C5`(개입점 역신호 전수 점검) → `G1` 지시 대기.

**병행 가능 조합**(파일 영역 겹침 0 — 착수 전 반드시 확인, #284):
- `C5`는 전 화면 점검이라 다른 UI 작업과 겹칠 수 있음. 착수 전 대상 파일 확정할 것
- ⚠️ **파일 영역 소유권**: `src/app/products/page.tsx` → 🌸 Cowork 전용 /
  `*.strings.ko.json` → 💻 Code 전용. 남의 영역 파일은 손대지 말고 보고(#284)
