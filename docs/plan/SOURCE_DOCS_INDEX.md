...
| 꽃틔움_가든_네이버_상품명_진단_엔진과_SEO_훅문구_개선(2026-06) | 상품명 9원칙 신호등, 이벤트필드(혜택형)/상세헤드라인(감성형) 분리, 경쟁강도 공식 | #33, #56, #48(부분) | 반영됨(2026-09-22 코드실측 재확인 — 상품명진단 9원칙/경쟁강도10:1~30:1/이벤트필드수치혜택검증 전부 확인·근본수정#56 완료. 상세헤드라인(감성형)은 #47/#48 미착수 영역으로 정확히 대응) |
| 한국어_이커머스_소싱의_카테고리_전환_동음이의어_필터링 | 카테고리 전환 수식어 사전 6개 축(신체부위/동물/장소/기기/연령/소재축소어) | #55(신규) | 반영됨(2026-09-22 curl실측 재확인 — 축B동물/축C기기는 이미 정상, 축A신체부위는 "귀청소기" 실제 오분류 재현했으나 근본원인이 코드결함 아닌 네이버 카테고리 데이터공백으로 확정, #55로 정직기록·근본수정보류) |
