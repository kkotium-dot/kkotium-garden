...
>
> ⭐ **다음 새 채팅 시작 메시지 (ACTIVE)**: "GET-merge updateStock 배포 완료(2026-06-06, production e3ab753). 작업1: GET-merge 번들(updateStock/setProductOutOfStock/bulkUpdateStock 전부 getProduct→stockQuantity override→전체 PUT + CLAUDE.md §3-7) 배포 — 부분 PUT 위험 제거. 작업2: inspect statusType 거짓초록 교정(명화 SUSPENSION을 inSync:false로 정상 판정, production 실증 GREEN). 작업3: sync/route.ts·cron/daily의 channel-products 엔드포인트 오용→/v2/origin-products 교정 + cron 자동중지는 NAVER_AUTOSUSPEND_ENABLED 게이트(기본 off, 비가역 0). 실 PUT/OOS 0·tsc 0·build OK. ★남은 단계: (1) Desktop GET-merge dryRun 교차검증(updateStock {dryRun:true}) (2) **명화 statusType=SUSPENSION→SALE 해제**(실 drift 확정 — updateStock GET-merge 또는 update confirm:true, 비가역, 대표 승인) (3) 명화 이미지 반영(update confirm:true, 비가역, mainImage 승격 선결) (4) NAVER_AUTOSUSPEND_ENABLED 활성 여부 대표 결정 (5) `* 2.*` macOS 중복본 untracked 정리(#34)."
