...
## 다음 새 채팅 시작 메시지 — 2026-07-22(12) 🖥 Desktop 인계 ⭐ ACTIVE

```
꽃틔움 가든 — 🖥 Desktop 이어받기
BASELINE: main 88a588d 이후 docs 커밋 (== origin/main == prod, Vercel 200)
※ 이 인계는 위 SHA 시점 기준(#291). 착수 전 보드에서 READY 재확인.

[★유일한 READY = DESKTOP-1 (Desktop 전용)]
처분 판정 실데이터 7채널 검증
전용 인계 문서: docs/handoff/DESKTOP_HANDOFF_2026-07-22.md
  (주입 SQL · 7채널 확인표 · 원복 절차 · 금지사항 전부 포함)

왜 Desktop인가: Cowork는 Supabase 쓰기가 정책 차단이라
스텁·합성 데이터로만 검증했다. "공급처 단절 상품이 실제로 존재할 때
7개 채널이 모두 같은 수를 말하는가"는 아직 아무도 실측하지 못했다.

[읽을 순서]
1. docs/handoff/DESKTOP_HANDOFF_2026-07-22.md   ← 이 작업 정본
2. docs/plan/TASK_BRIDGE.md §2-B(3환경 능력표) · §3-A(작업 큐 보드)
3. docs/plan/PARALLEL_WORK_TRACKER.md rev80
4. docs/plan/PRINCIPLES_LEARNED.md #273~#294
   ※ 옛 원칙은 archive/PRINCIPLES_LEARNED_archived-2026-07-22.md (두 파일 grep)

[🖥 Desktop 능력 경계]
✅ Supabase SQL 읽기/쓰기 · Chrome 검증 · Vercel 조회 · 코드 읽기
✅ docs/handoff/ MD 직접 작성(#49)
❌ 코드 파일 쓰기 · git commit/push
→ 결함 발견 시 **고치지 말고** 위치·재현절차·기대동작을 인계에 기록

[🚫 절대 금지]
· 디스코드 실발송(크론 수동 실행) — 운영자에게 실알림이 간다
· 네이버 스토어 PUT/POST(#46) — 발행 게이트는 모달 여는 것까지만,
  "GO — 실제 반영" 버튼 금지
· 테스트 데이터 방치 — 원복(STEP 3) 필수

[이번 사이클 완료 — 처분 판정 6채널 정합]
#273 엔진 → #278 대기함 → #285 부활소 → #286 발행게이트 →
#290 대시보드 → #292 위젯 → #293 디스코드 → #294 앱알림·주간리포트
신뢰성: #287 일괄변경 · C5-c 조용한실패 · #288 라벨통일
규율: #283 독자경계 · #284/#289/#291 병행 · #292 마운트확인 · #294 체크리스트실행
결정: D2 폴링 유지(ON) · E3 죽은코드 B(현상유지)

[꼭 지킬 원칙 — 최근 신규]
#286 "준비됐는가"와 "지금 팔 수 있는가"는 다른 축. 모르는 것으론 막지 않는다
#287 일괄 작업은 규율이 느슨해진다 — allSettled + 부분실패 보고
     ★fetch는 HTTP 500에도 reject 안 함 → Promise.all+fetch는 res.ok 필수
#288 라벨은 붙여쓰기 고정(식별자), 문장은 자연스러운 한국어
#289 병행 순번은 충돌한다. SHA가 정본
#290 판정 로직이 PURE여도 입력 로더가 한쪽에만 있으면 서버 집계가 갈라진다
#291 인계는 작성 시점 스냅샷 — BASELINE SHA + 착수 전 보드 확인
#292 검증 전 "실제로 마운트되는가" 확인. 죽은 코드는 결함의 냉동고
#293 앱은 모든 채널에서 같은 말을 해야 한다. 알림이 가장 늦게 정합된다
#294 ★체크리스트는 만든 다음에 실제로 돌려야 값을 한다.
     만든 턴에 전수 실행 + "정상인 곳"도 분류 기록

[판정 기준 변경 시 점검 목록 (#293/#294 — 실행 검증됨)]
화면 목록 · 대시보드 카운트 · KPI · 알림(디스코드/앱 종/주간리포트) ·
크론 다이제스트 · 사이드바 배지 · 외부 API → 같은 턴에 grep으로 전수 실행

[기존 필수]
#29a Python으로 한글 편집 · #31 문서 1500줄 분할(무손실)
#46 스토어 GO는 운영자 클릭으로만 · #62 같은 결론은 한 곳에서만
#262 셀러 실무 용어 · #283 페르소나는 셀러 대면에만
...
#284 소유권은 동시 편집 방지이지 영구 배타 아님
#285 행동을 유도하는 화면에는 말리는 신호도 함께

[환경 — dev 파손 3대 원인 (Code/Cowork용)]
...
· 🖥 Desktop은 코드 미수정 — 검증·진단 전문

[보류] G1-B 꽃단장(운영자 정리 중) / z3c stash / 클로드디자인 v7 PDF
```

---

## 다음 새 채팅 시작 메시지 — 2026-07-22(11) 인계 (SUPERSEDED)

```
꽃틔움 가든 — 이어받기
BASELINE: main 4fc37c6 이후 docs 커밋 (== origin/main == prod, Vercel 200)
※ 이 인계는 위 SHA 시점 기준(#291). 착수 전 git pull + 보드 READY 확인.

[★현재 READY 0건 — G1-B(꽃단장)만 남았고 운영자 정리 중]
처분 판정 정합성은 화면·서버집계·알림 3채널 전부 완료(#278→#290→#293).

[읽을 순서]
1. docs/plan/TASK_BRIDGE.md §3-A 보드   ← 정본
2. docs/plan/PARALLEL_WORK_TRACKER.md rev79
3. docs/plan/PRINCIPLES_LEARNED.md #273~#293
   ※ 옛 원칙은 archive/PRINCIPLES_LEARNED_archived-2026-07-22.md (두 파일 grep)

[이번 사이클 완료 — 처분 판정 시스템 전 채널 정합]
· 처분 엔진 #273 → 대기함 #278 → 부활소 #285 → 발행게이트 #286
· 대시보드 카운트 #290 → 위젯 전수 #292 → **디스코드 알림 #293**
· 신뢰성: 일괄변경 #287 · 조용한 실패(C5-c) · 라벨 통일 #288
· 규율: 독자경계 #283 · 병행 #284/#289/#291 · 마운트확인 #292
· E3 죽은코드 13개 → B(현 상태 유지) 확정
...
#292 검증 전 "실제로 마운트되는가" 확인. 죽은 코드는 결함의 냉동고
#293 ★앱은 모든 채널에서 같은 말을 해야 한다. **알림이 가장 늦게 정합된다**
     — 알림 결함은 "안 울리는" 형태라 눈에 안 띄고, 앱을 안 열어도 받는
     유일한 채널이라 가장 위험하다

[판정 기준 변경 시 점검 목록 (#293)]
화면 목록 · 대시보드 카운트 · KPI · 알림(디스코드/이메일) · 크론 다이제스트 ·
사이드바 배지 · 외부 API 응답 → **같은 턴에 전부 확인**
...
[검증 표준]
· 검증 전 마운트 여부 확인(#292)
· 인계 문서는 증거가 아니다 — target을 직접 추출해 확인(#283)
· "코드가 틀렸다" 결론 전에 테스트가 틀렸을 가능성부터(#282/#288)
· 목록·배지·카운트 분기: DB 주입 대신 fetch 스텁 또는 임시 프리뷰 라우트
· 알림 로직: **발송 없이 대상 선정만** 합성 데이터로 검증(#293)
· 레이아웃: 스샷 + getBoundingClientRect 둘 다
· 비동기 렌더: 1.5초 이상 대기, 단일 단계 재현
· 결함 발견 시: 단건 수습 금지, 전 채널 확장까지가 한 세트
...
· src/app/products/page.tsx  → 🌸 Cowork 주담
· *.strings.ko.json          → 💻 Code 주담
· 상대 영역 필요 시 (1)큐 확인 (2)미커밋 확인 (3)인계 명시하면 가능
...
## 다음 새 채팅 시작 메시지 — 2026-07-22(10) 인계 (SUPERSEDED)

```
꽃틔움 가든 — 이어받기
BASELINE: main 8fa392c 이후 docs 커밋 (== origin/main == prod, Vercel 200)
※ 이 인계는 위 SHA 시점 기준(#291). 착수 전 git pull + 보드 READY 확인.

[★현재 READY 항목 0건 — 운영자 판단 대기 2건]
E3  죽은 컴포넌트 13개 처리 방침 (삭제 / 보존 / 되살림 중 택)
G1-B 꽃단장 UI 구조 개편 (운영자가 직접 정리 중)

[읽을 순서]
1. docs/plan/TASK_BRIDGE.md §3-A 보드   ← 정본
2. docs/plan/PARALLEL_WORK_TRACKER.md rev78
3. docs/plan/PRINCIPLES_LEARNED.md #273~#292
   ※ 옛 원칙은 archive/PRINCIPLES_LEARNED_archived-2026-07-22.md (두 파일 grep)

[이번 세션 완료]
· 발행 게이트 #286 · 일괄변경 신뢰성 #287 · 조용한 실패 색출(C5-c)
· 라벨 통일 #288 · 순번 충돌 #289 · 대시보드 카운트 정합 #290
· 인계 교차 방지 #291 · 위젯 전수 점검 + 죽은 코드 표식 #292
· D2 폴링 유지 결정 · G1-A 꽃단장 문구(💻 Code)
...
#289 병행 순번은 충돌한다. 식별자는 SHA가 정본
#290 판정 로직이 PURE여도 입력 수집 경로가 한쪽에만 있으면 서버 집계는 갈라진다
#291 인계는 작성 시점 스냅샷이다. BASELINE SHA 명시 + 착수 전 보드 확인
#292 ★검증 전 "실제로 마운트되는가"를 먼저 묻는다.
     죽은 코드는 결함의 냉동고 — 되살릴 때 이미 고친 결함이 함께 부활한다
...
[검증 표준]
· ★검증 전 마운트 여부 확인(#292):
  grep -rn "Component" src --include="*.tsx" | grep -v "그_파일" | grep -vE "^\s*//"
· 인계 문서는 증거가 아니다 — 작업 전 target을 직접 추출해 확인(#283)
· "코드가 틀렸다" 결론 전에 테스트가 틀렸을 가능성부터(#282/#288 실증)
· 목록·배지·카운트 분기: DB 주입 대신 fetch 스텁 또는 임시 프리뷰 라우트
· 레이아웃: 스샷 + getBoundingClientRect 둘 다
· 비동기 렌더: 1.5초 이상 대기, 단일 단계 재현
· 결함 발견 시: 단건 수습 금지, 전 페이지·전 컴포넌트 확장까지가 한 세트
...
[보류] E3 죽은코드(판단대기) / G1-B 꽃단장(정리중) / z3c stash / 클로드디자인 v7
```

---

## 다음 새 채팅 시작 메시지 — 2026-07-22(9) 인계 (SUPERSEDED)

```
꽃틔움 가든 — 이어받기
BASELINE: main d1067ab 이후 docs 커밋 (== origin/main == prod, Vercel 200)
※ 이 인계는 위 SHA 시점 기준입니다(#291). 착수 전 git pull + 보드 확인.
로컬: /Users/jyekkot/Desktop/kkotium-garden

[읽을 순서]
1. docs/plan/TASK_BRIDGE.md §3-A 작업 큐 보드   ← 정본
2. docs/plan/PARALLEL_WORK_TRACKER.md rev77
3. docs/plan/PRINCIPLES_LEARNED.md #273~#291
   ※ 옛 원칙은 archive/PRINCIPLES_LEARNED_archived-2026-07-22.md (두 파일 grep)

[다음 — §3-A 보드 정본]
🌸 E2 나머지 대시보드 위젯 정합성 (READY)
   E1과 같은 결함(status로 세기 / 목적지 오류)이 DailyPlan·KkottiWidget·
   ZombieReactivationCard 등에 남아 있는지 전수 확인
⏸ G1-B 꽃단장 UI 개편 (HOLD — 운영자가 직접 정리 중. 스펙 나올 때까지 착수 금지)

[이번 세션 완료]
· 발행 게이트 #286 · 일괄변경 신뢰성 #287 · 조용한 실패 색출(C5-c)
· 라벨 통일 #288 · 순번 충돌 #289 · 대시보드 처분 카운트 정합 #290
· 인계 교차 방지 #291 · D2 폴링 유지 결정 · G1-A 꽃단장 문구(💻)
...
#289 병행 순번은 충돌한다. 식별자는 SHA가 정본
#290 ★판정 로직이 PURE여도 입력 수집 경로가 한쪽에만 있으면 서버 집계는 갈라진다.
     로더도 서버·클라 양쪽에 만들고, 동일 계산 함수를 재사용하라
#291 ★인계는 작성 시점 스냅샷이다. BASELINE SHA 명시 + 착수 전 보드 확인
...
## 다음 새 채팅 시작 메시지 — 2026-07-22(8) 인계 (SUPERSEDED)
...
```
꽃틔움 가든 — 이어받기
prod HEAD: 4a1955a 이후 docs 커밋 (== origin/main == prod, Vercel 200)
로컬: /Users/jyekkot/Desktop/kkotium-garden

[읽을 순서]
1. docs/plan/TASK_BRIDGE.md §3-A 작업 큐 보드   ← 정본
2. docs/plan/PARALLEL_WORK_TRACKER.md rev75
3. docs/plan/PRINCIPLES_LEARNED.md #273~#287
   ※ 옛 원칙은 archive/PRINCIPLES_LEARNED_archived-2026-07-22.md (두 파일 grep)
...
· 발행 게이트 #286 — 준비도 100%면 공급 끊긴 상품도 발행되던 결함
· ★일괄변경 신뢰성 #287 — Promise.all 결과를 안 봐서 실패가 조용히 묻히고
  화면과 DB가 어긋나던 결함 + 판매중 전환 게이트 부재
· D2: 도매꾹 폴링 유지(ON)·DRAFT 포함 결정

[다음 — §3-A 보드 정본. 병행 가능]
🌸 C5-c Promise.all 결과 미사용 전수 grep (READY) — #287 확장, 조용한 실패 색출
🌸 F3  상태 라벨 표기 통일 (READY) — "판매 중" vs "판매중" 혼재
💻 G1-A 꽃단장 문구 정비 (READY) — 페르소나 37 + 개발 은어 9(#262)
        ⚠️ 착수 전 git pull 필수
⏸ G1-B 꽃단장 UI 구조 개편 (WAIT-OP — 스펙 미정의, 대표님 정의 필요)
...
· 레이아웃: 스샷 + getBoundingClientRect 둘 다
· 비동기 렌더: 1.5초 이상 대기, 단일 단계 재현
· ★"코드가 틀렸다" 결론 전에 테스트가 틀렸을 가능성부터 확인(#282/#287 실증:
  라벨이 "판매 중"인데 정규식이 "판매중"이라 옵션이 없는 줄 알았음)
· 결함 발견 시: 단건 수습 금지, 전 페이지·전 스크립트 확장까지가 한 세트
...
## 다음 새 채팅 시작 메시지 — 2026-07-22(6) 인계 (SUPERSEDED)
...
```
꽃틔움 가든 — 이어받기
prod HEAD: 0648c49 이후 docs 커밋 (== origin/main == prod, Vercel 200)
로컬: /Users/jyekkot/Desktop/kkotium-garden

[읽을 순서]
1. docs/plan/TASK_BRIDGE.md §3-A 작업 큐 보드   ← 정본
2. docs/plan/PARALLEL_WORK_TRACKER.md rev73
3. docs/plan/PRINCIPLES_LEARNED.md #273~#285
   ※ 옛 원칙은 archive/PRINCIPLES_LEARNED_archived-2026-07-22.md (두 파일 grep)
...
· ★병행 세션 규율 #284 — 남의 미커밋 파일 손대지 않기 (실증)
· ★부활소 처분 권고 #285 — 되살릴지 묻는 화면에 "왜 죽었나"가 없었음
· D1 페르소나 DONE (잔여 3파일은 G1 종속)

[다음 — §3-A 보드 정본]
C5 개입점 역신호 전수 점검  (READY)
   #285를 전 화면으로 확장: "발행하기"·"소싱하기"처럼 행동을 유도하는
   화면마다 그 행동이 틀린 선택이 되는 조건이 보이는지 점검
G1 꽃단장 UI 개편          (WAIT-OP, 페르소나 잔여 3파일 37문구 흡수)

[꼭 지킬 원칙]
#29a 한글 다량 편집은 Python 스크립트로 · #31 문서 1500줄 분할(무손실 검증)
#46  스토어 GO는 운영자 클릭으로만 · #62 같은 결론은 한 곳에서만
...
#283 페르소나는 셀러 대면 앱 UI에만. 고객 대면 상품 카피는 금지
#284 병행 세션은 git add -A 금지. 남의 미커밋은 stash 금지·보고
#285 ★행동을 유도하는 화면에는 그 행동을 말려야 할 신호도 함께.
     개입점을 늘릴 때 로직은 늘리지 않는다(판정은 단일 권위 재사용)
...
## 다음 새 채팅 시작 메시지 — 2026-07-22(4) 인계 (SUPERSEDED)
...
```
꽃틔움 가든 — 이어받기
prod HEAD: b2a3a80 (== origin/main == prod, Vercel 200)
로컬: /Users/jyekkot/Desktop/kkotium-garden

[읽을 순서]
1. docs/plan/TASK_BRIDGE.md §3-A 작업 큐 보드   ← 무엇을 할지 정본
2. docs/plan/PARALLEL_WORK_TRACKER.md rev71
3. docs/plan/PRINCIPLES_LEARNED.md #273~#282
   ※ #31 분할됨 — 옛 원칙은 archive/PRINCIPLES_LEARNED_archived-2026-07-22.md
     원칙 찾을 땐 두 파일 함께 grep

[직전 완료 — 전부 배포·검증완]
· 처분 권고 엔진 #273 / 배지 레일 #274~#276 / 판매중지+원클릭 #277
· 처분 결정 대기함 #278 — status 필터라 공급처 단절 상품이 빠져 있던 것 수정
· dev 환경 근본 수정 #279 — 셸 NODE_ENV=production이 범인
· 배지 레일 모바일 확장 #280 — 모바일에 배지가 아예 없던 정보 비대칭 해소
· R-3 종결 #281 — 패널 열림 시 컬럼 축소(밀어내기로는 물리적 불가였음)
· R-1·R-2 통과 확인 (자사 제외 정상 / 하단 버튼 3개)
· PRINCIPLES 분할 #31 — 무손실 검증 PASS

[다음 — §3-A 보드 정본]
C4 재활성화 목록에 처분 권고 노출  (READY, 현재 상품명 배지만 있음)
G1 꽃단장 UI 개편                  (WAIT-OP, 착수 시 D1 11~13 흡수)
D1 페르소나 1~10+14~15             (IN-PROGRESS, 💻 Code 병행)

[꼭 지킬 원칙]
#29a 한글 다량 편집은 Python 스크립트로
#31  문서 1500줄 임계 시 분할. 분할은 반드시 무손실 검증
#46  스토어 PUT/GO는 운영자 클릭으로만. 앱 자동 실행 금지
#62  같은 결론은 한 곳에서만
#265 수치 PASS ≠ 화면 정상
#275 화면 그럴듯해도 수치 0일 수 있다(스샷+DOM실측 둘 다)
#277 판단과 실행을 잇되 방아쇠는 사람이. no-op은 실행경로 숨김
#278 대기함은 상태가 아니라 판정으로 모은다
#279 빌드는 되는데 dev만 안 되면 환경을 먼저 본다
#280 ★같은 데이터를 두 화면이 그리면 아이템 구성은 한 함수에서
#281 ★공간이 부족하면 밀어내기가 아니라 줄이기. 그리드 열 숨김은
     트랙·헤더라벨·셀개수를 반드시 함께 분기(타입검사 미검출)
#282 ★React 렌더 측정은 커밋 완료를 기다린다. "버그다" 결론 전 단일 단계 재현

[검증 표준]
· 목록·배지·카드 분기: DB 주입 대신 fetch 스텁 또는 임시 프리뷰 라우트
· 레이아웃: 스샷 + getBoundingClientRect 둘 다
· 비동기 렌더(ResizeObserver/SWR): 1.5초 이상 대기 후 측정, 단일 단계 재현
...
## 다음 새 채팅 시작 메시지 — 2026-07-22(2) Cowork 인계 (SUPERSEDED)
