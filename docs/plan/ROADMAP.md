...
---
## 다음 새 채팅 시작 메시지 — 2026-05-12 Session D (Sprint 6-A UI Phase 3 잔여)

<!-- session-d-handoff-short v1 -->

```
꽃틔움 가든 — Session D 시작.

docs/plan/PROGRESS.md → ROADMAP.md → SESSION_LOG.md 정독하고, 아래 STEP 0 환경 점검 후 현재 상태 + Session D 디테일 계획을 브리핑해주세요. 본 작업 시작은 제 Y/N 승인 후 진행.
...
- Sprint 6-A UI Phase 3 잔여 = 씨앗 심기 minq>1 경고 + getItemDetail 백엔드 보강 + admin 수동 폴링 트리거 + mark-oos Naver Commerce API 연결
- ROADMAP T1 분할 Session C-1에서 완료 (1351줄 → live 345 + archive 1019)
...
## Session D 작업 디테일 (인계 메시지 본문에서 분리 — 새 세션이 정독 시 흡수)

### 작업 범위 — Sprint 6-A UI Phase 3 잔여 4개

1. **씨앗 심기 `minq>1` 경고** — `src/app/products/new/page.tsx`
   - prefill 결과 영역에 minq 조건부 배너
   - minq>=2: 노란 배너 "위탁판매 불가 — 최소발주 N개. 묶음 상품 등록 또는 직접 재고 보유 필요"
   - minq>=5: 빨간 배너 (단 등록 버튼 disable은 시니어 정책으로 *제거* — 경고만, 셀러 자율성 보호)
2. **`getItemDetail(productNo)` 백엔드 보강** — `src/lib/sources/domemae-adapter.ts`
   - 도매꾹 OpenAPI v4.5 `getItemView` single item (minq 포함)
   - prefill API에서 호출해 minq를 정확히 InventorySnapshot에 기록 (현재 100건 묶음 응답에 minq 누락 → 1로 fallback)
3. **admin 수동 폴링 트리거** — `POST /api/admin/poll-inventory-now`
   - 세션 인증 + `pollAppRegisteredInventory()` 호출 + 결과 카운트 응답
   - dashboard `LowStockAlertWidget` 헤더에 "수동 폴링" 버튼 추가 + 결과 토스트
4. **`mark-oos` Naver Commerce API 연결** — admin confirm 모달 단계
   - 현재 `mark-oos`는 DB만 (의도된 안전 디자인)
   - admin 모달: "네이버 스토어에도 품절 상태로 전환할까요?" Y/N → `productStatusType: OUTOFSTOCK` 호출
   - 위탁판매 상품 한정 (수동 등록 자사 상품은 *셀러가 직접 처리*)
5. 검증 + commit + push + verify-vercel-deploy.sh --wait
...
- #26 IA 점검 (`/products/new` 6탭 영역 + 시각 일관성)
- #29 (a~e++) 한글 처리 6+1 규칙
- #31 MD 1500줄 자동 점검 — ROADMAP/PROGRESS는 분할 후 안전, SESSION_LOG 1102줄 T1 초과 (T2 1500 미달, 권고만)
...
- **Session E**: 6-B + 6-C (가격 변동 + 다른 셀러 추적 + 공급사 누적 평가)
...
---
## Session C 작업 디테일 (인계 메시지 본문에서 분리 — 새 세션이 정독 시 흡수)

### 작업 범위 — Sprint 6-A UI Phase 3 + ROADMAP 분할
...
4. **`mark-oos` Naver Commerce API 연결** — admin confirm 모달 단계
   ...
   - admin 모달: "네이버 스토어에도 품절 상태로 전환할까요?" Y/N → `productStatusType: OUTOFSTOCK` 호출
   - 위탁판매 상품 한정 (수동 등록 자사 상품은 *셀러가 직접 처리*)
5. **ROADMAP.md T1 분할** — 작업원칙 #31 (b)
   - 현재 1281줄
   - `docs/plan/archive/ROADMAP_2026Q2_MAY.md`로 deprecated 인계 메시지들 이전 (현 ROADMAP의 deprecated section + 옛 누적 시작 메시지)
   - 본 파일 = 최신 인계 + sibling 섹션 + Sprint 일정 + 보류 트랙 + 시크릿 (~600줄 목표)
   - idempotent marker 추가
6. 검증: TSC + build + 한글 grep + dev smoke
7. commit + push + verify-vercel-deploy.sh --wait
8. MD 갱신 — Session C 결과 + Session D 인계
...
- #26 IA 점검 (`/products/new` 6탭 영역 + 시각 일관성)
- #29 (a~e++) 한글 처리 6+1 규칙
- #31 (b) MD 의미 단위 자동 분할 — **본 세션 ROADMAP T1 분할 의무 작업**
- #32 push 전 `npm run build` 의무
- #33 useSearchParams Suspense 자동 점검
- #34 명백한 오류 파일 발견 시 사용자 알림
- #35 한글 사전 분리 패턴
