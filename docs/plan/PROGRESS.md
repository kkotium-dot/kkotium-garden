...
> 2026-07-03 (세션·Code) **SF-4a(860px 미리보기)+SF-3b(카피 영속) DONE — 빌드완·prod 검증 대기(Desktop) + 원칙 #189 박제** (Code turn·경로A HTML 조립·#55/#181/#46/#184/#188). 권위 SF4_DETAIL_EXPORT_SPEC. DetailAssemblyBoard: detail_images 순서 + 섹션 카피(편집)를 네이버식 860px detailContent 미리보기로 조립(신규 PURE lib detail-assemble.ts — buildDetailPreviewHtml/buildDetailCopyText·register buildDetailContent 패턴 미러). 데스크톱/모바일(860/360) 토글 + 규격 인디케이터(가로·이미지 수·카피 자수). SF-3b: SF-3a 제안→섹션별 편집→'상세 카피 저장'(명시적)으로 Product.description에 평탄화 텍스트 영속(신규 필드 0·#184 detail_images 평면 URL 유지·description은 발행 시 detailContent 본문). verify-first(#181): register/route buildDetailContent emit 구조 + PUT /api/products(sanitizeProductWrite allowlist) 실측 후 배선. detailContent HTML은 발행 시 계산(발행경로 무변경·#46 GO 게이트). 게이트 tsc0/build0·sentinel0·PURE 스모크(카피텍스트 빈섹션 생략 #82·860px 인터리브·빈→''). **원칙 #189**(네이버 상세=HTML 조립 우선): 상세페이지는 detailContent HTML 방식 — 이미지 합성(SF-4b)보다 HTML 조립이 텍스트 SEO 인덱싱·유지보수 유리. 프리미엄 이미지 합성은 옵션 분리. SF-4b(canvas/Firefly 합성)=후속. 대기: Desktop prod 미리보기·860px·detailContent 갱신(GET)·씨앗심기 정합 검증(#88) · JOURNAL-2(운영자 범위 Q).
> 2026-07-03 (세션·Code) **SF-3a DONE(섹션별 카피 제안·읽기전용·무료 템플릿) — 빌드완·prod 검증 대기(Desktop)** (Code turn·#55/#155/#181/#82). DetailAssemblyBoard 7섹션 각각에 카피 제안(헤드라인+본문) 표시 + 섹션별 복사 버튼. 신규 PURE lib section-copy.ts(buildSectionCopies·buildTemplateCopy 재사용): hook=골든키워드 훅(hookPhrase/seo_hook_text) · value=키워드 베네핏 · spec=상품데이터(카테고리/원산지/판매가) · usage=카테고리 템플릿 · trust=store_settings(A/S·문의·반품/교환비) · cta=구매유도 템플릿(무료배송/선물포장) · notice=공통안내 핀. 무료 우선(#155·AI 0)·소스 없으면 available=false→'생성 필요'(무날조 #82)·영속 0(SF-1 패턴·클립보드 복사만). verify-first(#181): 실 route(/api/products/{id}·/api/store-settings) + Product/StoreSettings 필드 shape 실측 후 매핑. PURE 스모크(풀데이터=7섹션 제안·희소데이터=value/spec/trust/notice 생성필요·hook/usage/cta 템플릿 초안). tsc0/build0·이모지0·sentinel0. SF-3b(편집+영속)=SF-4 내보내기와 함께 후속. 대기: Desktop prod 배양실 탭 섹션별 제안 노출 검증(#88).
