...
> 2026-07-06 (세션·Code) **ORDER-SYNC 근본수정(#62) — 상태전이 캡처 last-changed 3-endpoint flow + 6시간 cron — Code 라이브 검증 / Desktop prod 검증 대기 + 원칙 #192/#193 박제** (Code turn·#62/#82/#181/#46·읽기전용·주문 변이 0). 권위 ORDER_SYNC_FIX_SPEC. ★verify-first(#181)가 스펙 전제를 반증: 스펙/naver-doctor 권고 'from→lastChangedFrom 리네임'을 라이브 실측한 결과 product-orders 엔드포인트가 400 '`from 필드는 필수값 입니다`' 반환 — 즉 그 엔드포인트는 `from` 필수·생성일 기준이라 상태전이 캡처 불가. 정답=last-changed 3-endpoint flow(스펙이 대안으로 명시한 경로·라이브 검증): (1) GET product-orders/last-changed-statuses?lastChangedFrom&lastChangedTo(≤24h·초과 시 104140) → 변경 productOrderId (2) POST product-orders/query {productOrderIds} → 상세{order,productOrder,delivery,currentClaim?}(평면·content 래퍼 없음) (3) dedup+upsert(기존 STATUS_MAP/claim 매핑·평면 shape 매퍼). api-client: getOrders는 from/to로 복원(product-orders=생성일·대시보드 today-summary 정상 유지)+잘못된 주석 정정, 신규 getChangedOrderIds/getOrderDetails. route: 3-endpoint flow·honest total-failure gate(#82) 유지(전 윈도우 실패 OR 전 detail배치 실패 커버). cron: 신규 /api/cron/order-sync 6시간 주기(hours=48 lookback)·vercel.json crons 등재 — ★cron/daily는 이미 주문 sync(hours=24) 호출 중이었음(스펙 '미편입' 정정)이나 param 버그로 고장 상태였고 본 수정으로 정상화·6시간 cron은 빈도 보강(#193). 라이브 검증(proxy): manual=1&hours=720→success·32윈도우·changed/synced 캡처·NAVER_DIAG 에러0 / cron 라우트 인가200·미인가401. tsc0/build0. **원칙 #192**(주문 동기화 근본): 상태전이 캡처는 생성일(from/to) 아닌 변경분(last-changed-statuses→query) 3-endpoint flow. ★진단 도구(naver-doctor) 권고도 라이브 검증 필수—본 건 진단 권고가 라이브로 반증됨(#181·추측/미검증 진단 적용 금지). **원칙 #193**(동기화류 수동+자동 병행): 수동 전용은 stale—cron 병행·주문은 주기 짧게(플랜 daily 상한 내 최대—staggered ~2회/일). ★스펙 정정 권고(Desktop): ORDER_SYNC_FIX_SPEC §0.9.1/§1.1의 param-rename 전제는 라이브 반증됨→3-endpoint flow(§1.2 대안)가 정답. Desktop 스펙 갱신 권고. ORDER-SYNC 병행 스케줄 등재. 대기: Desktop prod 검증(넓은 윈도우 상태전이 반영·구매확정/취소 카운트 네이버 일치·cron 자동 갱신·#45).
