...
| **P1-A** | **상품 정식 카테고리 정비** — `naver_categories` 0행·전 상품 `category_id` 0건 → 드롭십 적합도·소싱 씨앗·네이버 검색 적합도 3곳 동시 무력화. 마스터 적재→제안 품질(D3/D4)→dryRun 백필→앱 개입점 4단계 | ✅ UCE-1~7로 카테고리 제안 100% 달성(`a878aef`). `naver_categories` 잔재 DDL만 별건 대기 | Step1~3: `naver_categories`·backfill 스크립트 / Step4 UI는 별도(파일 무겹침) | Desktop(설계 완료)→Code(구현) | 권위: `docs/design/PRODUCT_CATEGORY_BACKFILL_2026-08-20.md` + `docs/design/UCE7_MATCH_QUALITY_2026-08-27.md`. **2026-08-27 정정**: 원래 병목이던 "제안 품질(D3/D4)"은 UCE-1~7(정적 마스터 `NAVER_CATEGORIES_FULL` 직접매칭 + 명사중심 재설계 + 저신뢰 AI 교차확인)로 **프로덕션 12/12 실측 완료** — 더는 `naver_categories`(Prisma `NaverCategory`) 테이블 적재를 기다릴 이유가 없다(그 테이블의 유일한 앱 소비자였던 `auto-mapper.ts` matchCategory는 도달 불가능한 죽은코드로 이미 제거됨, rev127). 남은 것은 `naver_categories` 테이블 자체의 0행 상태 정리(DDL/시딩 여부)뿐이며 이는 앱 기능과 무관한 별건 — 운영자 판단 대기(삭제할지, 다른 용도로 채울지). **구현 착수 금지 해제됨(카테고리 제안 축은 완료)**. |
