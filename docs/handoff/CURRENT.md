...
- **status**: ✅ 부분재연동(#2) 안전장치 긴급 보강 완료(카테고리코드 wipe 위험 근본수정) → ✅ import route 필드 완전성 갭 10건 판정 완료(5건 매핑 구현, 5건 "네이버 API에 대응 없음/내부개념"으로 확정). 백필 스크립트 하나로 통합, dry-run만 실행(--apply는 미실행, Desktop 검토 대기). **confirm:true 실행은 여전히 하지 않음.** ⚠️ **아침 소싱 알림 실발송은 미해결(8/12 미발송 재확인) — 아래 정정 섹션 참조.**
...
6. [계측 — 미해결] 아침 소싱 알림 실발송: 발생 시점에 Vercel Recent Invocations 직접 확인
   (재조사 금지 목록 5건은 다시 파지 말 것 — 위 정정 섹션 참조)
...
## ⚠️ 아침 소싱 알림 실발송 — 미해결 · 계측 중 (2026-08-12 정정)

> **정정 사유(2026-08-12)**: 직전에 이 섹션은 "실발송·DB저장 검증 완료 / 3일 미스터리 종결"로 기록돼 있었으나 **사실이 아니다**. 8/12 아침에도 아침 소싱 알림 **미발송이 재확인**됐다. self-fetch 제거(rev120)가 원인의 일부였을 수는 있으나 실발송이 실제로 정상화됐다는 증거는 없다. 상태를 **미해결 — 계측 중**으로 되돌린다. (원칙 #310 연장 — 인계문서의 "확인됨/완료" 서술도 재검증 대상.)

- **현재 상태**: 매일 아침 KST 08:00 꼬띠 소싱 추천 Discord 알림이 **여전히 안정적으로 도착하지 않음**. 8/12 아침 미발송 재확인.
- **계측 우선(다음 액션)**: 실제 크론 invocation 유무를 Vercel 대시보드 Recent Invocations로 직접 확인(로그 1시간 보존 한계로 사후조회 불가 → 발생 시점에 계측 필요). "sent:true인데 DB엔 레코드 없음"의 재현이 아직 안 됨(rev120 기록).
...
- **self-fetch(자기 자신의 다른 API 라우트를 HTTP로 호출)는 별개 서버리스 함수 홉을 만든다**(#338) — 호출자의 maxDuration은 피호출 라우트에 적용되지 않는다. ⚠️ 단, self-fetch 제거가 아침 소싱 미발송을 **완전히 해결했다고 단정 금지** — 8/12 미발송 재확인, 실발송은 여전히 미해결(위 정정 섹션).
