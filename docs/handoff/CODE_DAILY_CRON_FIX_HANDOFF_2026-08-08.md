...
## ★★★★★★ 2026-08-10 최신 상태 — 정규 스케줄 미실행 재확인 (여전히 미해결)

**운영자 확인(8/10)**: 8/6(수)에 마지막 정상 발송, 8/7 이후 미발송 지속. 위 실행지시(maxDuration+독립크론분리)는 Code가 완료·배포함(fec8759).

**Desktop 3단계 검증 완료(8/8)**:
1. 웹훅 URL 직접 curl 3회 테스트 → 전부 정상 도착(순수 메시지·embed·avatar_url 포함 전부)
2. 프로덕션 환경변수 확인(임시 진단 엔드포인트) → `DISCORD_WEBHOOK_KKOTTI_RECOMMEND`가 정확히 그 웹훅 ID와 일치
3. **`sendDiscord()` 함수를 실제 코드 그대로 호출** → `{ok:true, status:204}` 반환 + **디스코드에 실제 도착 확인**
4. 신규 `/api/cron/sourcing-daily`를 CRON_SECRET 인증 헤더로 curl 직접 호출 → `{sent:true}` + **디스코드 정상 도착 확인**(8/8)

**결론**: `sendDiscord()` 함수·환경변수·웹훅·신규 크론 코드 자체는 전부 완전히 정상. **수동으로 부르면 100% 성공한다.**

**★★★ 그런데 정규 스케줄(08:00 KST 자동 실행)은 8/8 신규크론 배포 이후 8/9·8/10 두 번 다 미실행 확인**(DB에 정규 스케줄 시각으로 생성된 레코드 없음, 수동 curl로 만든 레코드만 존재. Vercel Cron Jobs 탭 "View Logs"도 최근 1시간 0건 — Hobby 로그보관 제약으로 정시 실행 여부 직접 확인은 여전히 불가하나, DB 부수효과 부재로 간접 확정).

**범위 재확정**: "함수 코드"는 완전히 정상(입증됨) · "함수를 수동으로 부르는 것"은 됨(입증됨) · **"Vercel 크론 스케줄러가 정시에 자동으로 이 함수를 부르는 것"은 여전히 안 됨**(신규 크론 분리 후에도 재발).

이건 Code가 최초 조사(rev114)에서 발견한 **"5개 크론 전부 7일간 실행 로그 0건"**과 정확히 같은 패턴이 신규 크론(`sourcing-daily`)에서도 반복되고 있다는 뜻이다. **개별 라우트 코드 문제가 원인이 아니라 크론 스케줄러 자체(또는 그 앞단)의 문제로 범위가 좁혀졌다.**

## ★★★★★★ 다음 조사 방향 (Code, 최우선 — 운영자 지시: Code 우선 배분)

1. **Vercel 크론 스케줄러 자체 상태 재점검**: 대시보드 "Enabled" 토글이 켜져 있어도(Desktop 확인) 실제로 스케줄러가 도는지 별도 확증 필요. Vercel 공식 문서에서 Hobby 플랜 크론이 "특정 조건에서 스킵될 수 있는 경우"(예: 프로젝트 비활성 기간, 크론 개수 제한 재확인 등)를 재검색.
2. **Vercel Cron Jobs 탭 각 항목 상세 페이지 재확인**: Desktop이 아직 "View Logs" 클릭 이후 뜨는 로그 페이지만 봤다 — 혹시 크론 이름 자체를 클릭하면 "Recent Invocations"나 "Last Run" 같은 별도 상세 페이지가 있는지 재확인(Desktop이 못 찾았을 가능성).
3. **대안: 외부 크론 트리거 도입 검토**: Vercel 자체 크론이 신뢰할 수 없다면, GitHub Actions의 scheduled workflow나 cron-job.org 같은 외부 서비스로 `sourcing-daily` 엔드포인트를 매일 정시에 curl 호출하는 우회 방법도 후보로 검토(임시 안전망). GitHub Actions는 이미 이 프로젝트가 GitHub에 있으니 `.github/workflows/`에 scheduled workflow 추가가 빠른 경로일 수 있음.

**운영자 방침(2026-08-10)**: Claude Code 우선 배분, Desktop과 병렬. Cowork는 가능 시 추가 활용. 실제 Discord 발송 테스트는 여전히 운영자 승인 필요.
