...
# ★ v3+ 개정 (2026-07-29) — CLAUDE.md/PRD 템플릿 대조 반영

> 근거: `docs/research/CLAUDE_PRD_TEMPLATE_COMPARISON_2026-07-29.md`
> 위키독스 347584(CLAUDE.md)·347587(PRD) 정독 후 채택분. §13~§17을 보강한다.

## 18. 설계문서 PRD 3요소 헤더 (PRD 전용 파일 대신)

PRD 템플릿의 고유 가치(비목표·수용기준·영향범위)는 실제 반복 문제(범위 확대·완료 판정 흔들림)에 답한다(#315 충족). 그러나 **전용 `PRD.md`는 신설하지 않는다** — 1인 운영에서 design/ 시점문서가 이미 요구사항을 담아 이중 관리가 된다(#313). 대신 **설계 시점문서 상단에 3블록을 표준 섹션**으로 넣는다.
...
- **비목표**가 있어야 범위 확대를 막는다(PUBLISH_REVIEW_GATE v1→v2 재작성 사례).
- **수용 기준**은 사람이 읽는 설명이 아니라 **테스트 가능한 조건**으로(게이트 완료="우회 0건" 같은).
- 모르는 요구사항은 추정하지 말고 **질문 목록으로** 분리(#303).

## 19. CLAUDE.md Done Criteria 블록 (흩어진 검증 규칙 통합)

완료 보고 형식을 CLAUDE.md에 고정(현재 관행이나 미문서화). 표준 템플릿(347584)의 Done Criteria 채택.

```
## 완료 기준
- 관련 테스트/검증을 실행했다(어디서 — localhost/프로덕션 명시 #305).
- 실행 못 한 검증은 "미검증(사유·재검증 시점)"으로 남겼다(#310).
- 변경 파일 + 위험도(🟢추가/🟡이동/🔴삭제)를 요약했다.
```

> 삽입은 CLAUDE.md 감축(§16 🔴)과 함께 운영자 승인 후. 이 문서엔 채택 결정만 기록.
