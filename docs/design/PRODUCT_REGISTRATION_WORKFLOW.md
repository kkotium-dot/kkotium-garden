...
## 1-A. 통합 노력 레인 (단순/디테일) ⊇ 공급사 자산품질 A/B + 씨앗심기 선행 게이트

> 권위 보강: `docs/handoff/HANDOFF_2026-06-21_dual-lane-workflow-and-seed-planting-gate.md`(rev2). 단일 통합 위계(#116·#117·#118).

**최상위 축 = 노력 레인(EFFORT LANE):**
- **단순 레인(Simple)** = 실제 발행/업로드가 쓰는 빠른 경로(크롭 기반 썸네일/상세). **실 발행 = 단순 레인.** 라이브 상품의 단순·스테일 대표이미지는 버그가 아니라 단순 레인이 의도대로 동작한 결과 — 개선 범위 = 단순 레인 대표 흰배경 마감(REP_FINISHING).
- **디테일 레인(Detail)** = 프리미엄 풀 파이프라인(Firefly 3-plane 자연합성·Nano Banana Pro). 검증되면 발행에 공급(현재 테스트 트랙).

**위계 — A/B는 별도 축이 아니라 레인 내부 서브라우팅으로 흡수(#116):**

```
EFFORT LANE (단순 | 디테일)            <- 최상위 축: 프리미엄 노력 정도 / 무엇을 발행하나
  └─ ASSET-QUALITY ROUTING (A | B)     <- 내부: 공급사 자산 재사용(A) vs 크롭/생성(B)
      └─ 씨앗심기 게이트 (sourcing)    <- 디자인 전 프런트 게이트(아래)
```

- A/B 권위 = `IMAGE_DETAIL_TWO_BRANCH_SYSTEM.md`(A=공급사 자산 양호→재사용 / B=빈약→크롭·생성). 이 문서는 폐기하지 않고 **각 레인 내부 자산품질 서브라우팅**으로 재배치(흡수).
- 앱: 상품별 레인 상태 + A/B 서브상태를 applyStatus / 개입 대기열에 노출(후속 C22).

**씨앗심기 선행 게이트(#117):** 가격/마진 검증은 씨앗심기(SEO/ROI 소싱 시드·크롤 기반)가 채운 소싱 정보에 의존한다. 올바른 순서: **크롤링 → 씨앗심기(소싱 채움) → (공급사 제공정보 + 씨앗심기 소싱) → 온실 아틀리에(디자인·STEP6)**. 디자인-전-소싱(명화 위반)은 가격을 검증 불가로 만든다 → 개입 카드 `sourcing_incomplete`(6장). 가격/마진 카드는 이 게이트에 종속.

**디테일 레인 11단계(운영자 정수)** — 표준 7단계에 **STEP10 디자인 커넥터 품질개선**을 추가:
1 SEO/ROI 씨앗심기 · 2 썸네일 컨셉+상세 틀 · 3 썸네일·상세 생성 · 4 누끼 · 5 Firefly 3-plane 합성(Nano Banana Pro) · 6 폴더 적재 · 7 MD 일괄+병렬 · 8 썸네일·상세 활용 · 9 상세 추가이미지 · **10 디자인 커넥터(Figma/Canva/Adobe/Claude Design) 품질개선** · 11 발행(최후·비가역 #46).
단순 레인 = 압축 부분집합(4~5 Firefly 합성 + 10 커넥터 생략·빠른 크롭→발행). 재입고 미정 옵션은 품절 유지(#118).
...
| 카드 | 트리거 | 해소 |
|---|---|---|
| `sourcing_incomplete` | 소싱(SEO/ROI 씨앗심기) 미비 상태로 디자인 진행 → 판매가/마진 검증 불가(#117) | 씨앗심기 백필(크롤 소싱) → 가격/마진 확정 |
