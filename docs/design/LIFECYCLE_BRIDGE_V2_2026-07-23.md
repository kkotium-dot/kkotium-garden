...
| 축 | 질문 | 산출 | 소비처 |
|---|---|---|---|
| **상태(진단)** | 지금 무엇인가 | `LifecycleState` **7종** | 배지·필터·카운트 |
...
1. **[선행]** `lifecycle.ts` 신설 — 실제 원자 → **7상태** 파생 단일함수. 기존 `source-gone.ts`/`sales-assets.ts` 재사용(중복 판정 금지). ※2026-07-23 `e2048ba`로 완료.
