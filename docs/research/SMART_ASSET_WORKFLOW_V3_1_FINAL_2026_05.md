...
### 1-1. 디자이너 관점 — 본 시스템의 가치

본 v3.1 FINAL은 *디자이너 감각 보존*을 핵심 가치로 설계됩니다. CTI 8축이 컨셉·톤을 자동 추론하고 12개 골격이 레이아웃을 미리 잡아두면, 디자이너는 매번 "누가 살까 / 어떤 색감으로 갈까 / 5섹션이 좋을까 3섹션이 좋을까"를 0부터 고민하지 않습니다. 시스템이 추론한 컨셉·톤·골격이 *출발점*이고, 디자이너는 (a) 그대로 OK 또는 (b) 1클릭으로 다른 골격으로 교체 또는 (c) 디테일에만 집중하면 됩니다. 이는 작업원칙 #40 (Designer Sense is the Sacred Resource) 의 구체적 구현입니다.
...
- Groq Llama 3.1 8B: 무료, 상품당 약 500 토큰 = 1초 미만 (1차 추론, 일상 90% 케이스)
- Claude Vision 보강(D 등급 또는 신뢰도 <70%): **모델 디폴트 `claude-haiku-4-5`** (이미지당 약 ₩10, $0.007/이미지 기준), **옵션 `claude-sonnet`** (정밀 진단 필요 시, 약 5배 비용)
- 새싹 단계 월 비용 예상: 약 **₩2,000~5,000** (월 100상품 × 이미지 3장 = 300건 중 보강 호출 일부, PDF Part 3 사용 케이스 표 기준)
- `ANTHROPIC_MONTHLY_CAP` 환경변수로 월 한도 cap, 도달 시 Sharp 4축 + Groq 룰 기반 단독 작동 (자동 fallback 회로)
...
interface DiagnosisResult {
  ...
  inferenceConfidence: number; // 0-100 (>=90 자동 진행, 70-89 확인 필요, <70 디자이너 직접 선택)
  diagnosedAt: Date;
}
```
...
### 4-A. 골격 분포 (어디에 가장 자주 적용)

이커머스 상세페이지는 패턴이 정해져 있어 *12개*로 새싹~파워 단계 SKU 전체를 커버합니다:

- 새싹 셀러의 주력 SKU **80%**: `S1` · `S2` · `S5` 3개 골격 (일상·리빙·식품)
- 베스트셀러 후보 **15%**: `S3` · `S6` · `S10` (선물·감각형)
- 전문 상품 **5%**: `S4` · `S7` · `S12`

8개로 줄이면 표현 다양성 부족, 20개로 늘리면 CTI 자동 매칭 정확도 저하 — *12개가 다양성과 정확도의 균형점*. 근거는 PDF Part 2 #4 참조.

### 4-B. 골격 확장성 — S13+ 추가 가능

12개는 *절대 변경 불가가 아닙니다*. 새싹 → 파워 단계로 성장하면서 본인 매장에 자주 쓰이는 패턴이 보이면 `S13` · `S14` 등을 디자이너가 직접 추가할 수 있도록 시스템이 설계됩니다:

- 각 골격 = Tailwind 토큰 + JSON 명세 (`src/lib/automation/layout-skeletons/sNN-*.ts`)
- 골격 추가 작업 = 코드 변경 1~2시간 (skeleton-matcher 매칭 규칙 1줄 + JSON 명세 1 파일)
- CTI 추론은 신규 골격을 *자동 후보*에 추가 (matcher rule만 update)

### 4-C. CTI → 골격 매칭 실 작동 예시 (3가지 검증 케이스)

본 3가지 예시는 PDF Part 1 #3에서 추출. `concept-tone-inference.ts` 의 *unit test 시드*로 활용 권장.

**예시 A — 유아용 실리콘 식판 4종 세트, 1만 5천원**

| 축 | 자동 추론 결과 |
|---|---|
| 페르소나 | `kidsmom` ("유아용" + 식판 카테고리) |
| 사용 맥락 | `daily` (선물 키워드 없음) |
| 가격 포지션 | `budget` (식판 평균 ₩25,000의 60%) |
| 상품 유형 | `set` ("4종 세트" 명시) |
| 컬러 무드 | `warm` (실리콘 식판 = 파스텔 톤) |
| 감성 톤 | `friendly` (kidsmom + budget 매트릭스) |
| 사진 스타일 | `white` (식판 = 누끼) |
| 장르 | `korean` (kidsmom + 식품류) |

→ 자동 선택 골격: **`S5`** (가성비·일상·세트 / 친근·비비드·한국형)

**예시 B — 독일 명품 향초, 15만원, 단일 사이즈**

| 축 | 자동 추론 결과 |
|---|---|
| 페르소나 | `30-40s` (향초 + 고가) |
| 사용 맥락 | `gift` + `daily` 혼합 |
| 가격 포지션 | `premium` (향초 평균 ₩30,000의 5배) |
| 상품 유형 | `single` |
| 컬러 무드 | `calm` (명품 향초 = 다크톤 패키지) |
| 감성 톤 | `sensory` (premium + 30-40s) |
| 사진 스타일 | `detail` (질감 클로즈업이 핵심) |
| 장르 | `minimal` |

→ 자동 선택 골격: **`S6`** (표준·선물·단일 / 감각·라이프·미니멀) 또는 **`S10`** (프리미엄 일상 감각)

**예시 C — 전동 드릴 18V, 부품 7종 포함, 8만원**

| 축 | 자동 추론 결과 |
|---|---|
| 페르소나 | `30-40s` (공구 카테고리) |
| 사용 맥락 | `pro` ("18V", "부품" 키워드) |
| 가격 포지션 | `standard` |
| 상품 유형 | `options` (부품 7종) |
| 컬러 무드 | `mono` (블랙·메탈릭) |
| 감성 톤 | `professional` (pro + 30-40s) |
| 사진 스타일 | `white` (공구 = 누끼) |
| 장르 | `minimal` |

→ 자동 선택 골격: **`S4`** (표준·전문·단일 / 전문·화이트·미니멀)

본 3건의 *예상 결과*가 `skeleton-matcher.ts` 테스트의 정합성 검증 기준이 됩니다.
