...
- date: 2026-06-13 / 세션7-g rev2 (Desktop)
...
전상품 적용: 슬롯마다 레인 태깅. AUTHENTIC-ART 레인이 비는 상품(비명화)도 PHOTOREAL 룰(회화 마감 금지)은 보편 적용. 앱 개입점 = 사실성 레인 가드(§7).

---

## 1. 전체 톤 = "하루의 빛 시퀀스" (실사)

4컷이 **같은 카메라 · 같은 실사 컬러그레이드 · 같은 구도 문법**으로 찍힌 한 편의 사진 시퀀스. 차이는 오직 하루의 빛(시간).
- 통일축: 실사 카메라(Sony A7 50mm f/1.8) + filmic 컬러그레이드 + 상단중앙 여백(병 합성 자리)
- 차별축: 시간대 빛 / 색온도 / 명도 곡선 `중상 → 최상 → 중 → 중하`
- ★ 마감 = 실사 사진. 유화/회화/일러스트 마감 금지(#71). "하루의 빛"과 팔레트는 실제 조명·실제 사물에서 나온다.
근거: 상세페이지가 이미 "하루의 시간 아크"("하루의 시간을 향으로 고르세요 / 아침에서 늦은 오후까지")를 채택.
...
## 5. 모델 선택 (이미지별 최선 · 디폴트 금지)

| 단계 | 모델 | 이유 |
|---|---|---|
| 향 씬 / 히어로 / 라이프스타일 (신규 실사 씬 생성) | Adobe Firefly Image 5 | 실사 품질 + 상용 인뎀니티 — 라이브 스토어 법적 안전 |
| 합성 (실제 제품 누끼 + 씬 정합/harmonize) | Nano Banana Pro | 편집/합성 최강 |
| 진짜 명화 (라벨 · S5 스토리) | 모델 불필요 | 퍼블릭도메인 실제 이미지 사용 |

설정: Portrait 3:4(→ 인앱 Sharp 4:5 크롭) · 2K+ · 그라운딩 ON.
★ 매 컷 직전 참조 0 강제 — 직전 실패의 1순위 원인(참조 잠금 → cotton에 april 꽃 유입). 4컷 교차오염 차단.
...
- ★ 플레이스홀더 문구 개정: 기존 "A 인상주의 + C 정물" → "실사 정물 — [장면]" (#71: 향 씬에 회화 레이어 금지)
- 팔레트 정합 필수: 생성컷이 위 CSS 색역에 들어와야 페이지가 한 덩어리로 읽힘
- 히어로: 기존 "A. 모네 수련 무드(Firefly)" → 실사 프리미엄 환경 + (선택) 벽면 실제 모네(퍼블릭도메인) 액자. 명화 연결은 실제 라벨/실제 모네가 짊어짐
- S5 스토리: 퍼블릭도메인 진짜 모네 이미지 사용(AUTHENTIC-ART 레인)
...
앱 개입점(자연스러운 human-in-the-loop · #56):
- 자산 슬롯마다 realism lane 태그(authentic_art / photoreal) 부여
- 관제탑/스튜디오에 사실성 가드: PHOTOREAL 슬롯에 회화/AI-유화 마감 감지 시 경고(기존 main_image_white_bg·fidelity 가드와 동형)
- AUTHENTIC-ART 슬롯은 퍼블릭도메인 실제 작품 소스만 허용(저작권 게이트 연동)
- 비명화 상품: authentic_art 레인 비어도 photoreal 룰은 보편 적용

향후 대상: 달항아리 도어벨 `cmp3afb450001gng5468w0qpc`, 아이스트레이 `cmpp62yje00015xup5h8pgwx0`.
