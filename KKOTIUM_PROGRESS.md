...
| Task | 내용 | 상태 | 비용 |
|------|------|------|------|
| C-6 | 원산지코드 518건 자동완성 (naver-origin-codes.ts 로컬 상수) | ⬜ 대기 | 무료 |
| C-7 | 카테고리 4,993건 정합성 검증 + 속성 매핑 테이블 | ⬜ 대기 | 무료 |
| C-8 | 상품 속성 완성도 체커 (카테고리별 필수/선택 속성 점수화) | ⬜ 대기 | 무료 |
...
| Task | 내용 | 상태 | 비용 |
|------|------|------|------|
| C-4 | 수익성 분석 대시보드 (유입경로별 수수료 + 순이익 시뮬레이터) | ⬜ 대기 | 무료 |
| C-10 | 배송 자동화 확장 (자동 발주확인 + 수량 클레임 대응) | ⬜ 대기 | 무료 |
| C-3 | 대량 등록 배치 파이프라인 (소싱보관함 10개 일괄) | ⬜ 대기 | 무료 |
...
| API 키 교체 | 노출된 Gemini 3개 + Groq 키 교체 권장 |
