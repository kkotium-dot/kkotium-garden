  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (data.success && data.products && data.products.length > 0) {
        ...
        console.log('✅ 검색 성공:', data.products.length, '개');
      } else {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      for (const product of selectedProducts) {
        const retailPrice = Math.round(product.price * 2.5); // 150% 마진
      ...
      if (successCount > 0) {
        alert(`✅ ${successCount}개 상품이 저장되었습니다!${errorCount > 0 ? `\n⚠️ ${errorCount}개 실패` : ''}`);
        
        // 저장 성공한 상품 제거
        const newProducts = products.filter(p => !selected.has(p.id));
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">
            키워드로 빠르게 상품을 수집하세요 (최대 20개)
          </p>
          <Link
            href="/sourced"
            className="inline-block mt-4 text-sm text-pink-600 hover:underline font-semibold"
          >
            → 수집된 상품 보기
          </Link>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
          <div className="flex gap-4">
            <input
              ...
              placeholder="검색 키워드 입력 (예: 여성 원피스, 남성 셔츠, 니트)"
        ...
        {!loading && !error && products.length === 0 && (
          <div className="text-center py-20">
            <div className="text-6xl mb-4">🎯</div>
            <p className="text-xl text-gray-600">검색어를 입력하고 상품을 찾아보세요</p>
            <p className="text-sm text-gray-500 mt-2">
              예: 여성 원피스, 남성 셔츠, 니트, 가디건, 블라우스
            </p>
          </div>
        )}
      </div>
