...
import type { ExcelGenerateOptions } from '@/lib/excel/naverExcel.types';
// EXCEL_PRECHECKER_2026-09-16 — toNaverProduct moved to lib/naver/to-naver-
// product.ts so both this route and /api/naver/excel-precheck can share the
// exact same transform (Next.js route.ts files may only export GET/POST/etc —
// even a re-export of another function is rejected at build time, confirmed
// by `next build`'s route-type checker, which is stricter than `tsc --noEmit`
// alone — so this is a plain import, not a re-export).
