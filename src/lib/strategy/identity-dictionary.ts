...
export const COMPOUND_NOUNS: ReadonlyArray<string> = [
  ...
  // 결함2 골든셋 검증(2026-08-20, seed-keywords.ts)에서 드러남: "꽂이"로
  // 끝나는 흔한 상품명(우산꽂이·칫솔꽂이·펜꽂이)이 전부 여기 해당한다.
  '꽂이',
  // UCE-11(2026-09-04, extractNouns 실행 확정): 같은 -이 어미 충돌인데
  // "목걸이"는 위 '걸이' 부분매치로 우연히 보호받고 "넥타이"는 무방비였다
  // ("넥타이"→"넥타"로 스트리핑, docs/design/
  // UCE11_TOKENIZER_LONGNAME_CANDIDATES_2026-09-04.md §A). 카탈로그 밖
  // 20종 전수 dryRun으로 확인한 나머지 실제 표준어 -이 어미 상품 식별자만
  // 보수적으로 추가(#353 고신뢰만). "다듬이/부지깽이"(희귀·구식)·
  // "쓰레받이"(표준어 아님, 정식은 "쓰레받기")·"화분받이/냄비받이"(정식은
  // "화분받침/냄비받침" — 이미 -침으로 끝나 이 버그 영향 밖)는 표준어·빈도
  // 확신 부족으로 제외.
  '넥타이', '손잡이', '재떨이', '손톱깎이', '딸랑이', '지팡이',
];
...
export const GENERIC_MODIFIERS: ReadonlyArray<string> = [
  ...
  // 결함2 수정(2026-08-20, seed-keywords.ts): extractNouns()가 이 단어들을
  // 명사로 통과시켜 소싱 씨앗 도출에서 headword로 오인되는 게 실 DB로 확인됐다
  // ("듀얼 무선 가습기..." → nouns[0]="듀얼"). identity-extractor의 명사 목록에도
  // 똑같이 새는 문제라 공유 사전에 추가한다(seed-keywords 전용 사본을 두지 않음).
  '듀얼', '라이트', '저소음', '실용적인', '무선', '휴대용', '다용도',
  '접이식', '양방향', '초경량', '스마트', '멀티', '올인원',
  // 결함2 미세 결함(2026-08-20, Desktop 리뷰): "기능"(월 1,890)이 검색량
  // 임계값을 넘어 씨앗으로 통과했으나 상품이 아니라 일반명사다. 같은 부류의
  // 일반명사(용도·타입·스타일)도 함께 차단 — '디자인'은 위에 이미 있음.
  '기능', '용도', '타입', '스타일',
];
