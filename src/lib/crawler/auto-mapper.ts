...
async function matchCategory(title: string, description: string) {
  ...
  const scores = categories.map(cat => {
    ...
    const levels = [cat.depth1, cat.depth2, cat.depth3, cat.depth4]
      .filter(Boolean)
      .map(l => l!.toLowerCase());
    ...
    // 정확히 일치하는 경우 보너스
    if (title.toLowerCase().includes(cat.depth4?.toLowerCase() || '')) {
      score += 50;
    }
  ...
  return {
    code: best.category.categoryCode,
    fullPath: best.category.fullPath || '',
    level1: best.category.depth1 || '',
    level2: best.category.depth2 || undefined,
    level3: best.category.depth3 || undefined,
    level4: best.category.depth4 || undefined,
...
function generateCategoryReasoning(category: any, title: string, score: number): string {
  const levels = [category.depth1, category.depth2, category.depth3, category.depth4]
    .filter(Boolean);

  if (score > 50) {
    return `상품명에 "${category.depth4 || category.depth3}"가 포함되어 ${levels.join(' > ')} 카테고리로 매칭했습니다.`;
  } else if (score > 20) {
    return `상품명과 설명에서 ${levels.join(', ')} 키워드를 발견하여 매칭했습니다.`;
  } else {
    return `유사한 키워드를 기반으로 ${category.depth1} 카테고리로 분류했습니다.`;
  }
}
...
async function matchOrigin(
  ...
  for (const origin of origins) {
    const region = origin.originName.toLowerCase();

    // 정확히 일치하는 경우
    if (text.includes(region)) {
      return {
        code: origin.originCode,
        region: origin.originName,
        confidence: 1.0,
        source: (specs && Object.keys(specs).some(k => k.includes('원산지'))) 
          ? 'specs' as const 
          : (description.includes(origin.originName) 
