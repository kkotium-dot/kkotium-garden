...
export default function TodayQueue() {
  ...
  const allItems: QueueItem[] = [
    ...
    },
    {
      ...
      color: 'var(--info)', bg: 'var(--info-bg)', border: 'var(--info)',
    },
  ];
      ...
      <div className="kk-hero" style={{ padding: '18px 20px', display: 'flex', flexDirection: 'column', gap: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 32, height: 32, borderRadius: 9, background: '#FEF0F3' }}>
            <ListChecks size={18} style={{ color: '#F63B28' }} />
          </div>
  ...
  return (
    <div className="kk-card" style={{ padding: '18px 20px', display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <span className="kk-pop-num" style={{ fontSize: 12, fontWeight: 700, color: '#F63B28', background: '#FFF0EF', border: '1px solid #FFD6D3', borderRadius: 99, padding: '3px 10px', whiteSpace: 'nowrap' }}>
      ...
      <Link href={primary.href} style={{ textDecoration: 'none' }}>
        ...
        >
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            {/* Ink text on the semantic tint = AA-safe (bright semantics fail as
                small text on light bg; the tint + colored icon carry the signal). */}
            <p style={{ margin: 0, fontSize: 15, fontWeight: 800, color: '#2A1F1A', lineHeight: 1.3 }}>
              {primary.label}{' '}
              ...
              <Link key={item.key} href={item.href} style={{ textDecoration: 'none' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 12px', borderRadius: 10, background: '#fff', border: '1px solid #F0E0E5', cursor: 'pointer' }}>
                  ...
                  <p style={{ margin: 0, fontSize: 13, fontWeight: 700, color: '#3A3A3A', flex: 1, minWidth: 0 }}>
                    {item.label}{' '}
                    <span style={{ fontVariantNumeric: 'tabular-nums', color: '#2A1F1A', fontWeight: 800 }}>{item.count}</span>{strings.unit}
                  </p>
